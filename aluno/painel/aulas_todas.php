...
plano_aula_data, plano_aula_texto, plano_aula_conteudo, plano_aula_atividade, plano_aula_video, plano_aula_google_form, plano_aula_publicado, plano_aula_hash, func_id, func_nome, disciplina_id, disciplina_nome 
FROM smc_plano_aula
...
WHERE plano_aula_id_turma = '$row_Matricula[vinculo_aluno_id_turma]' AND plano_aula_publicado = 'S' AND plano_aula_data <= NOW() AND (plano_aula_conteudo IS NOT NULL OR plano_aula_video IS NOT NULL OR plano_aula_atividade IS NOT NULL OR plano_aula_google_form IS NOT NULL) $disciplinaQry
...
<div class="container">
 ...
 <h5><strong>Conteúdo das aulas (<?php echo $totalRows_Conteudo; ?>)</strong></h5>
 <hr>
 <a href="index.php" class="waves-effect waves-light btn-small btn-flat"><i class="material-icons left">arrow_back</i>voltar</a>
 <a href="calendario.php" class="waves-effect waves-light btn-small btn-flat"><i class="material-icons left">today</i>Calendário</a>
 <a href="disciplinas.php" class="waves-effect waves-light btn-small btn-flat"><i class="material-icons left">apps</i>DISCIPLINAS</a>
<hr>
 ...
 <table class="striped1">
   ...
   ?>
     ...
     <tr>
	   ...
	   <blockquote>
	   <?php echo inverteData($row_Conteudo['plano_aula_data']); ?> - <?php echo $row_Conteudo['func_nome']; ?> - <?php echo $row_Conteudo['disciplina_nome']; ?>
	   <h6><?php echo $row_Conteudo['plano_aula_id']; ?> - <?php echo $row_Conteudo['plano_aula_texto']; ?></h6>
	   ...
	   <?php $totalPalavras = str_word_count($row_Conteudo['plano_aula_conteudo']); $totalMinutos = ceil($totalPalavras/30); ?>
		...
		<?php if  ($totalMinutos > 1) { ?>
	  	Tempo para leitura: <strong><?php $totalPalavras = str_word_count($row_Conteudo['plano_aula_conteudo']); echo ceil($totalPalavras/50); ?> minutos</strong> (sem considerar imagens, vídeos e anexos).
		<?php } ?> 
	   ...
	   <a class="waves-effect waves-light btn-small right btn <?php if ($totalRows_Visualizou > 0) { ?><?php } else { ?> red<?php } ?>" href="aulas_conteudo.php?aula=<?php echo $row_Conteudo['plano_aula_hash']; ?>"><?php if ($totalRows_Visualizou > 0) { ?>VISUALIZADA<?php } else { ?>ACESSAR<?php } ?></a>
