...
$maxRows_Logins = 30;
$pageNum_Logins = 0;
if (isset($_GET['pageNum_Logins'])) {
  $pageNum_Logins = $_GET['pageNum_Logins'];
}
$startRow_Logins = $pageNum_Logins * $maxRows_Logins;

mysql_select_db($database_SmecelNovo, $SmecelNovo);
$query_Logins = "SELECT login_aluno_id, login_aluno_id_aluno, login_aluno_data_hora, login_aluno_ip, login_aluno_ano FROM smc_login_aluno WHERE login_aluno_id_aluno = '$row_Matricula[vinculo_aluno_id_aluno]' ORDER BY login_aluno_id DESC";
$query_limit_Logins = sprintf("%s LIMIT %d, %d", $query_Logins, $startRow_Logins, $maxRows_Logins);
$Logins = mysql_query($query_limit_Logins, $SmecelNovo) or die(mysql_error());
$row_Logins = mysql_fetch_assoc($Logins);

if (isset($_GET['totalRows_Logins'])) {
  $totalRows_Logins = $_GET['totalRows_Logins'];
} else {
  $all_Logins = mysql_query($query_Logins);
  $totalRows_Logins = mysql_num_rows($all_Logins);
}
...
if (!empty($_SERVER['QUERY_STRING'])) {
  ...
  foreach ($params as $param) {
    if (stristr($param, "pageNum_Logins") == false && 
        stristr($param, "totalRows_Logins") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_Logins = "&" . htmlentities(implode("&", $newParams));
  }
}
...
<div class="container">
 ...
 <h5><strong>Frequência online</strong></h5>
 <hr>
...
<table class="striped">
  ...
  <?php do { ?>
    <tr>
      <td class="center"><?php echo date('d/m/Y', strtotime($row_Logins['login_aluno_data_hora'])); ?></td>
      <td class="center"><?php echo date('H\:i', strtotime($row_Logins['login_aluno_data_hora'])); ?></td>
    </tr>
...
<table border="0">
  <tr>
    <td width="25%" class="center"><?php if ($pageNum_Logins > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_Logins=%d%s", $currentPage, 0, $queryString_Logins); ?>">Primeira</a>
        <?php } // Show if not first page ?></td>
    <td width="25%" class="center"><?php if ($pageNum_Logins > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_Logins=%d%s", $currentPage, max(0, $pageNum_Logins - 1), $queryString_Logins); ?>">Anterior</a>
        <?php } // Show if not first page ?></td>
    <td width="25%" class="center"><?php if ($pageNum_Logins < $totalPages_Logins) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_Logins=%d%s", $currentPage, min($totalPages_Logins, $pageNum_Logins + 1), $queryString_Logins); ?>">Próxima</a>
        <?php } // Show if not last page ?></td>
    <td width="25%" class="center"><?php if ($pageNum_Logins < $totalPages_Logins) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_Logins=%d%s", $currentPage, $totalPages_Logins, $queryString_Logins); ?>">Última</a>
        <?php } // Show if not last page ?></td>
  </tr>
</table>
    </div>
