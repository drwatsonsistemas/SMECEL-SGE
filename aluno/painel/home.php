...
<body class="<?= $row_Perfil['aluno_cor_fundo'] ?> lighten-4">
  ...
  <div id="containerId" class="container">
    ...
    <script>
      ...
      var hashLogado = <?php echo json_encode($row_AlunoLogado['aluno_hash']); ?>;
            ...
            if (response.length > 0) {
              ...
              response.forEach(function (postagem) {
                ...
                var deleteButtonHtml = '';
                    if (postagem.aluno_hash === hashLogado) {
                        deleteButtonHtml = `<a class="right red-text" style="cursor:pointer" onclick="deletarPostagem(${postagem.id_aluno_postagem})"><i class="material-icons">delete</i></a>`;
                    }


                var postagemHtml = `
                            <div class="row" id="postagem-${postagem.id_aluno_postagem}">
                              <div class="col s12">
                                <div class="card">
                                  ...
                                  <div class="card-action">
                                    ...
                                    <!--<a style="cursor:pointer" onclick="deletarPostagem(${postagem.id_aluno_postagem})"><i class="material-icons">delete</i></a>-->
                                    ${deleteButtonHtml}
                                  </div>
                                </div>
                              </div>
            ...
            success: function (response) {
              if (response.success) {
                ...
                if (response.postagem) {
                  var postagemHtml = `
                            <div class="row" id="postagem-${response.postagem.id_aluno_postagem}">
                              <div class="col s12">
                                <div class="card">
                                  <div class="card-content">
                                    <div class="row valign-wrapper" style="margin-bottom: 0;">
                                   ...
                                   <div class="card-action">
                                    ...
                                    <a href="#"><i class="material-icons">share</i></a>
                                    <a style="cursor:pointer" onclick="deletarPostagem(${response.postagem.id_aluno_postagem})"><i class="material-icons">delete</i></a>
                                  </div>
                                </div>
                              </div>
                            </div>`;
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            // Realiza a requisição AJAX para deletar a postagem
            $.ajax({
                url: 'funcoes/delete_post.php', // URL do script PHP que lida com a exclusão
                type: 'POST',
                data: { id_postagem: idPostagem },
                dataType: 'json', // Esperamos uma resposta JSON
                success: function (response) {
                    if (response.success) {
                        // Remove o elemento da postagem deletada
                        //$('#postagem-' + idPostagem).fadeOut();

                        $('#postagem-' + idPostagem).fadeOut(1500, function() {
                            $(this).remove();
                        });
                        ...
                        Swal.fire({
                            ...
                            icon: 'success',
                            showConfirmButton: false, // Mostra o botão de OK
                            confirmButtonText: 'OK',
                            timer: 1000, // Fecha o alerta automaticamente após 1 segundo
                            allowOutsideClick: true, // Desabilita clique fora do alerta
                ...
                error: function (jqXHR, textStatus, errorThrown) {
                    console.error('Erro ao deletar postagem:', textStatus);
                    Swal.fire(
                        'Erro!',
                        'Houve um problema ao deletar a postagem.',
                        'error'
                    );
                }
            });
