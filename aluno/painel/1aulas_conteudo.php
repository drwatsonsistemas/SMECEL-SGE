...
plano_aula_anexo_atividade_caminho, plano_aula_anexo_atividade_data_hora, plano_aula_anexo_atividade_resposta_professor, plano_aula_anexo_atividade_visualizada_professor, plano_aula_anexo_atividade_visualizada_aluno 
...
<div class="container">
 ...
 <a href="aulas_conteudo_imprimir.php?aula=<?php echo $colname_Conteudo; ?>" target="_blank" class="waves-effect waves-light btn-small btn teal"><i class="material-icons left">print</i>IMPRIMIR</a>
	  ...
	  <hr>
        ...
        <?php } ?>
		<br><?php echo $row_Conteudo['func_nome']; ?><br><small><?php echo $row_Conteudo['disciplina_nome']; ?></small>
		</div>
      ...
      <blockquote>
		...
		<?php if  ($totalMinutos > 1) { ?>
	  	<p>Tempo médio de leitura: <strong><?php $totalPalavras = str_word_count($row_Conteudo['plano_aula_conteudo']); echo ceil($totalPalavras/50); ?> minutos.</strong> (Sem considerar anexos, vídeo e atividades)</p>
		<?php } ?> 
	  ...
	  <?php if ($row_Conteudo['plano_aula_conteudo']<>"") { ?>
		...
		<?php echo $row_Conteudo['plano_aula_conteudo']; ?>
...
?>
        ...
        <div class="card-panel light-green lighten-5">
		  ...
		  <strong>Instruções:</strong> 
			...
			Clique no botão abaixo para enviar a foto da sua atividade já respondida.<br>
			...
			Tente tirar a foto em um ambiente iluminado e que mostre as informações com clareza.
          ...
          <p><a class="waves-effect waves-light btn modal-trigger" href="#modal1">ENVIAR RESPOSTA</a></p>
		  
		  <?php } else { ?>
		  
          <p><span class="waves-effect waves-light btn disabled">ENVIO BLOQUEADO</span></p>
		  <i>Prazo para envio da atividade expirou em <?php echo inverteData($row_Conteudo['plano_aula_atividade_resposta_obrigatoria_data_limite']); ?>. <br>Entre em contato com seu professor.</i>
		  
		  
		  <?php } ?>
		  
          <?php if ($totalRows_AtividadeResposta > 0) { // Show if recordset not empty ?>
            <h6>Seus arquivos enviados</h6>
			<?php do { ?>
              <p>
			  <a href="<?php echo URL_BASE.'anexos_respostas/'.$row_Conteudo['plano_aula_id'] ?>/<?php echo $row_AtividadeResposta['plano_aula_anexo_atividade_caminho']; ?>" target="_blank"><i class="material-icons tiny">file_download</i> Enviado em <?php echo date('d/m/Y \à\s H\hi', strtotime($row_AtividadeResposta['plano_aula_anexo_atividade_data_hora'])); ?></a> 
			  -
			  <?php if ($row_AtividadeResposta['plano_aula_anexo_atividade_visualizada_professor']=="N") { ?>
			  <span class="orange"><small>AGUARDANDO VISUALIZAÇÃO DO PROFESSOR</small></span>
			  <?php } else { ?>
			  <?php echo $row_AtividadeResposta['plano_aula_anexo_atividade_resposta_professor']; ?>
			  <?php } ?>
			  <hr>
			  </p>
        ...
        <?php } ?>
		  ...
		  <?php if ($totalRows_Registrar < 1) { ?>	
	  	  ...
	  	  <form method="post" name="form3" action="" autocomplete="off" class="card-panel center">
              ...
              <div class="file-field input-field col s12">
				Se você considera que já concluiu todas as atividades propostas nesta aula, clique no botão abaixo.
              </div>
		  ...
		  Aula marcada como concluida. Parabéns! <i class="material-icons left">check_circle</i> 
      ...
      <table class="striped1 highlight">
        <?php do { ?>
          <tr>
            ...
            <td valign="top" style="vertical-align: top;"><small style="font-size:10px;"> <a href="#"><?php echo current( str_word_count($row_Comentarios['aluno_nome'],2)); ?>
              ...
              </a> <?php if ($row_Comentarios['com_at_aluno_duvida']=="S") { ?><span class="orange"> Dúvida </span><?php } ?><span class="right"><?php echo date('H\hi - d/m/Y', strtotime($row_Comentarios['com_at_aluno_data_hora'])); ?></span> </small>
        ...
        <?php } ?>
                ...
                <strong>Professor(a) <?php echo current( str_word_count($row_Conteudo['func_nome'],2)); ?> <?php $word = explode(" ", trim($row_Conteudo['func_nome'])); echo $word[count($word)-1]; ?> respondeu:</strong>
            ...
            <td width="20"><?php if ($row_Comentarios['com_at_aluno_id_matricula']==$row_Matricula['vinculo_aluno_id']) { ?>
              <a href="javascript:func()" onclick="confirmaExclusao('aula=<?php echo $row_Conteudo['plano_aula_hash']; ?>&deletar=<?php echo $row_Comentarios['com_at_aluno_id']; ?>')"><i class="material-icons grey-text lighten-3">delete_forever</i></a></a>
              <?php } ?></td>
          </tr>
        ...
        <table>
          <tr>
            <td width="70" class="top" valign="top" style="vertical-align: top;"><?php if ($row_AlunoLogado['aluno_foto']=="") { ?>
              ...
              <img src="<?php echo URL_BASE.'aluno/fotos/' ?><?php echo $row_AlunoLogado['aluno_foto']; ?>" width="100%" class="hoverable">
              <?php } ?></td>
            <td class="top" valign="top" style="vertical-align: top;">
			<div class="lead emoji-picker-container">
                <textarea id="conteudoDuvida" style="min-height:80px;" name="com_at_aluno_comentario" class="white form-control textarea-control materialize-textarea" data-emojiable="true"  data-length="255" id="textarea1" required></textarea>
                <label for="textarea1">
                  <?php if ($totalRows_Comentarios == 0) { ?>
                  Seja o primeiro a comentar o assunto acima <br><span class="caracteres">255</span> caracteres restantes <br>
                  <?php } else { ?>
                  Escreva um comentário sobre o assunto <br><span class="caracteres">255</span> caracteres restantes<br>
                  <?php } ?>
                </label>
              </div>
	  ...
	  <label class="black-text">
        ...
        <span>Marque esta opção se seu comentário é uma dúvida que precisa ser respondida pelo professor</span>
      </label>
	  </p> 
  ...
  <small id="loaded_n_total"></small>
          ...
          <!--
		  <form id="atividadeEnviar" method="post" enctype="multipart/form-data" name="form2" action="<?php  ?>" autocomplete="off">
            
              <div class="file-field input-field col s12">
                <div class="btn"> <span>ARQUIVO/FOTO</span>
                  <input type="file" name="plano_aula_anexo_atividade_caminho" class="panel" value="" accept="image/*, application/pdf, application/msword, application/vnd.openxmlformats-officedocument.wordprocessingml.document" required>
                </div>
                <div class="file-path-wrapper">
                  <input class="file-path validate" type="text">
                </div>
              ...
              <div class="input-field col s12">
                <input type="submit" id="btnAtividade" value="ENVIAR" class="waves-effect waves-light btn">
              </div>
			  
           
            <input type="hidden" name="MM_insert2" value="form2">
            <input type="hidden" name="plano_aula_anexo_atividade_id_atividade" value="<?php echo $row_Conteudo['plano_aula_id']; ?>">
            <input type="hidden" name="plano_aula_anexo_atividade_id_aluno" value="<?php echo $row_Matricula['vinculo_aluno_id']; ?>">
          </form>
		  
		  
		  
	<div id="envioAtividade" class="row col s12" style="display:none">
...
-->
		    ...
		    <div class="row">
              <div class="input-field col s12">
                <div class="card-panel"> <span class="blue-text text-darken-2"> 
				  Envie a resposta para essa atividade, caso seja solicitado pelo seu professor.
                  Acesse seu painel através de um celular e tire a foto da atividade com a câmera, ou envie uma foto que você já tirou e que está na galeria do seu aparelho.
                  Você pode escolher o arquivo que esteja em seu computador ou notebook. </span> </div>
              </div>
            </div>
		  
        </div>
        <div class="modal-footer"> <a href="#!" class="modal-close waves-effect waves-green btn-flat">VOLTAR</a> </div>
      </div>
    </div>
  </div>
</div>
...
function uploadFile(){
	...
	ajax.open("POST", "envio_atividade.php?aula=<?php echo $row_Conteudo['plano_aula_id']; ?>&aluno=<?php echo $row_Matricula['vinculo_aluno_id']; ?>");
	ajax.send(formdata);
}  
...
$( "#atividadeEnviar" ).submit(function( event ) {
