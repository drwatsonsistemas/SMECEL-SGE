...
FROM smc_catraca
WHERE catraca_id_matricula = :cod AND catraca_data = :data AND catraca_tipo = :tipoS");
$duplicado->bindParam(':cod', $_POST['registro_id_aluno']);
$duplicado->bindParam(':data', date('Y-m-d'));
$duplicado->bindParam(':tipoS', $tipo);
$duplicado->execute();
...
$entrada->bindParam(':cod', $_POST['registro_id_aluno']);
$entrada->bindParam(':data', date('Y-m-d'));
$entrada->bindParam(':tipoE', $tipoVerifica);
$entrada->execute();
$resultadoEntrada = $entrada->rowCount();
$linhaEntrada = $entrada->fetch( PDO::FETCH_ASSOC );
...
if (empty($cod)) {
    ...
    //die;
} elseif ($resultadoEntrada < 1) {
    echo "<div class=\"card-panel amber darken-2 center-align\"><h5>Não houve registro de entrada para a matrícula informada.</h5></div>";
    //die;
	...
	<div class=\"row\">
      <div class=\"col s4\">
			<img src=\"../../aluno/fotos/{$foto}\" width=\"250\" class=\"responsive-img\">
	  </div>
      <div class=\"col s8\">
			<h5 class=\"card-panel amber darken-2 center-align\">
			SAÍDA JÁ REGISTRADA NESTA DATA! <i class=\"material-icons\">report_problem</i>
			</h5>
	...
	<div class=\"card-panel amber darken-5 center-align\"><h6>Não será realizado mais de um registro de saída por dia.</h6></div>";
    //die;
} else {
...
<h5 class=\"center\">Últimas Saídas</h5>
