...
switch ($row_func['func_grupo_sanquineo'].$row_func['func_fator_rh']) {
	
	case "A+":
	$tipoQry = " AND ( (func_grupo_sanquineo = 'A' AND func_fator_rh = '+') OR (func_grupo_sanquineo = 'A' AND func_fator_rh = '-') OR (func_grupo_sanquineo = 'O' AND func_fator_rh = '+') OR (func_grupo_sanquineo = 'O' AND func_fator_rh = '-') ) ";
	$descDoar = "A+, A-, O+, O-";
	break;
	
	case "A-":
	$tipoQry = " AND ( (func_grupo_sanquineo = 'A' AND func_fator_rh = '-') OR (func_grupo_sanquineo = 'O' AND func_fator_rh = '-') ) ";
	$descDoar = "A-, O-";
	break;
	
	case "B+":
	$tipoQry = " AND ( (func_grupo_sanquineo = 'B' AND func_fator_rh = '+') OR (func_grupo_sanquineo = 'B' AND func_fator_rh = '-') OR (func_grupo_sanquineo = 'O' AND func_fator_rh = '+') OR (func_grupo_sanquineo = 'O' AND func_fator_rh = '-') ) ";
	$descDoar = "B+, B-, O+, O-";
	break;
	
	case "B-":
	$tipoQry = " AND ( (func_grupo_sanquineo = 'B' AND func_fator_rh = '-') OR (func_grupo_sanquineo = 'O' AND func_fator_rh = '-') ) ";
	$descDoar = "B-, O-";
	break;
	
	case "AB+":
	$tipoQry = " ";
	$descDoar = "RECEBE DE TODOS";
	break;
	
	case "AB-":
	$tipoQry = " AND ( (func_grupo_sanquineo = 'A' AND func_fator_rh = '-') OR (func_grupo_sanquineo = 'B' AND func_fator_rh = '-') OR (func_grupo_sanquineo = 'O' AND func_fator_rh = '-') OR (func_grupo_sanquineo = 'AB' AND func_fator_rh = '-') ) ";
	$descDoar = "A-, B-, O-, AB-";
	break;
	
	case "O+":
	$tipoQry = " AND ( (func_grupo_sanquineo = 'O' AND func_fator_rh = '+') OR (func_grupo_sanquineo = 'O' AND func_fator_rh = '-') ) ";
	$descDoar = "O+, O-";
	break;
	
	case "O-":
	$tipoQry = " AND (func_grupo_sanquineo = 'O' AND func_fator_rh = '-') ";
	$descDoar = "O-";
	break;
	
		
	case 5:
	$tipoQry = " AND func_escolaridade = '5' ";
	break;
...
WHERE func_id <> '$colname_func' AND func_id_sec = '$row_Secretaria[sec_id]' AND func_situacao = '1' AND func_grupo_sanquineo <> '-1' AND func_fator_rh <> '-1' $tipoQry
...
<main class="ls-main">
  <div class="container-fluid">
    <h1 class="ls-title-intro ls-ico-home">RELAÇÃO DE DOADORES</h1>
    <!-- CONTEUDO -->
    
    <a href="rel_func_tipo_sanguineo.php" class="ls-btn">VOLTAR</a>
<BR><BR>

  <div class="ls-alert-info">
  Relação de funcionários que podem doar para:<br><br>
  <strong><?php echo $row_func['func_nome']; ?></strong><br>
  Tipo Sanguíneo: <strong><?php echo $row_func['func_grupo_sanquineo']; ?><?php echo $row_func['func_fator_rh']; ?></strong><br>
  Recebe de: <strong><?php echo $descDoar; ?></strong><br>
  Total de doadores compatíveis: <strong><?php echo $totalRows_FuncionariosGraduacao; ?></strong> 
  </div>
...
<?php if ($totalRows_FuncionariosGraduacao > 0) { // Show if recordset not empty ?>
  <table class="ls-table ls-sm-space">
    <thead>
      <tr>
        <th width="50"></th>
        <th class="ls-txt-center">FUNCIONÁRIO</th>
        <th class="ls-txt-center" width="200">TIPO SANGUINEO</th>
        <th class="ls-txt-center" width="150">IDADE</th>
        <th class="ls-txt-center" width="280">CONTATO</th>
        <th class="ls-txt-center" width="200">CPF</th>
        </tr>
    </thead>
    <tbody>
      <?php $num = 1; 
	  do { ?>
        <tr>
          ...
          <td class="ls-txt-center"><?php echo $row_FuncionariosGraduacao['func_grupo_sanquineo']; ?><?php echo $row_FuncionariosGraduacao['func_fator_rh']; ?></td>
          <td class="ls-txt-center"><?php echo idade($row_FuncionariosGraduacao['func_data_nascimento']); ?></td>
          <td class="ls-txt-center"><?php echo $row_FuncionariosGraduacao['func_telefone']; ?> <?php echo $row_FuncionariosGraduacao['func_celular1']; ?> <?php echo $row_FuncionariosGraduacao['func_celular2']; ?></td>
