...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf(
    "INSERT INTO smc_te_ponto (te_ponto_id_sec, te_ponto_descricao, te_ponto_endereco, te_ponto_num, te_ponto_bairro, te_ponto_latitude, te_ponto_longitude, te_ponto_obs) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form2")) {
  $updateSQL = sprintf(
    "UPDATE smc_te_ponto SET te_ponto_descricao=%s, te_ponto_endereco=%s, te_ponto_num=%s, te_ponto_bairro=%s, te_ponto_latitude=%s, te_ponto_longitude=%s, te_ponto_obs=%s WHERE te_ponto_id=%s",
    GetSQLValueString($_POST['te_ponto_descricao'], "text"),
    GetSQLValueString($_POST['te_ponto_endereco'], "text"),
    GetSQLValueString($_POST['te_ponto_num'], "text"),
    GetSQLValueString($_POST['te_ponto_bairro'], "text"),
...
<body onload="self.print();">
  ...
  <br>
  <h3 class="ls-txt-center">RELAÇÃO DE ALUNOS POR PONTOS</h3>
  <br>
  ...
  <?php if ($totalRows_Pontos > 0) { ?>
    ...
    <?php do { ?>
      ...
      mysql_select_db($database_SmecelNovo, $SmecelNovo);
      $query_VInculo = "
		SELECT vinculo_aluno_id, vinculo_aluno_id_aluno, vinculo_aluno_id_turma, vinculo_aluno_id_escola, vinculo_aluno_id_sec, vinculo_aluno_ano_letivo, vinculo_aluno_transporte, vinculo_aluno_ponto_id, vinculo_aluno_data, 
		vinculo_aluno_hash, vinculo_aluno_verificacao, vinculo_aluno_boletim, vinculo_aluno_situacao, vinculo_aluno_datatransferencia, vinculo_aluno_da_casa, vinculo_aluno_historico_transferencia, vinculo_aluno_vacina_atualizada, 
		vinculo_aluno_vacina_data_retorno, vinculo_aluno_conselho, vinculo_aluno_conselho_parecer, vinculo_aluno_internet, vinculo_aluno_multietapa, vinculo_aluno_rel_aval, vinculo_aluno_dependencia, vinculo_aluno_reprovado_faltas,
		aluno_id, aluno_nome, turma_id, turma_nome, turma_turno, escola_id, escola_nome,
		CASE turma_turno
		...
		WHERE vinculo_aluno_ponto_id = '$row_Pontos[te_ponto_id]' AND vinculo_aluno_ano_letivo = '$row_AnoLetivo[ano_letivo_ano]'
    ORDER BY aluno_nome ASC
      ...
      <div class="ls-box">
        <h3><?php echo $row_Pontos['te_ponto_descricao']; ?></h3><?php echo $row_Pontos['te_ponto_endereco']; ?>
        <?php echo $row_Pontos['te_ponto_num']; ?>     <?php echo $row_Pontos['te_ponto_bairro']; ?>
        <?php echo $row_Pontos['te_ponto_latitude']; ?>     <?php echo $row_Pontos['te_ponto_longitude']; ?>
        <br><br>
        ...
        <?php if ($totalRows_VInculo > 0) { ?>
          ...
          <table class="bordasimples" width="100%">
            ...
            <tbody>
              ...
              do { ?>
                <tr>
                  <td width="20" class="ls-txt-center"><?php echo $n;
                  $n++; ?></td>
                  <td width="200"><?php echo $row_VInculo['aluno_nome']; ?></td>
                  <td width="120" class="ls-txt-center"><?php echo $row_VInculo['turma_nome']; ?></td>
                  <td width="80" class="ls-txt-center"><?php echo $row_VInculo['turma_turno']; ?></td>
                  <td class="ls-txt-center"><?php echo $row_VInculo['escola_nome']; ?></td>
                </tr>
        ...
        <?php } else { ?>
          ...
          <i>Nenhum aluno cadastrado neste ponto.</i>
