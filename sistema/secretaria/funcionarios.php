...
func_formacao, func_situacao, func_foto, func_senha, func_senha_ativa, funcao_id, funcao_nome, id_regime, regime_nome
FROM smc_func
LEFT JOIN smc_regime ON id_regime = func_regime 
LEFT JOIN smc_funcao ON funcao_id = func_cargo
...
<body>
      ...
      <div class="ls-box ls-board-box">
        <?php if (isset($_GET["cadastrado"])) { ?>
          <div class="ls-alert-success ls-dismissable"> <span data-ls-module="dismiss" class="ls-dismiss">&times;</span>
            <strong>Aviso:</strong> Funcionário cadastrado com sucesso! </div>
        <?php } ?>
        <?php if (isset($_GET["editado"])) { ?>
          <div class="ls-alert-success ls-dismissable"> <span data-ls-module="dismiss" class="ls-dismiss">&times;</span>
            <strong>Aviso:</strong> Funcionário editado com sucesso! </div>
        <?php } ?>
        <?php if (isset($_GET["excluido"])) { ?>
          <div class="ls-alert-success ls-dismissable"> <span data-ls-module="dismiss" class="ls-dismiss">&times;</span>
            <strong>Aviso:</strong> Funcionário excluido com sucesso! </div>
        <?php } ?>
        <?php if (isset($_GET["erro"])) { ?>
          <div class="ls-alert-danger ls-dismissable"> <span data-ls-module="dismiss" class="ls-dismiss">&times;</span>
            <strong>Aviso:</strong> Ocorreu um erro na ação anterior. Um e-mail foi enviado ao administrador do sistema!
          </div>
        <?php } ?>
        <a href="funcionarios_cadastrar.php" class="ls-btn-primary ls-ico-plus">CADASTRAR NOVO FUNCIONÁRIO</a>
        <a href="impressao/rel_funcionarios.php?<?php echo $url_atual[1]; ?>" class="ls-btn"
          target="_blank">IMPRIMIR</a>
        <div class="ls-group-btn ls-group-active ls-float-right"> <a href="funcionarios.php?situacao=1"
            class="ls-btn-primary <?php if ($situacao == "1") { ?> ls-active<?php } ?>">ATIVOS</a> <a
            href="funcionarios.php?situacao=2"
            class="ls-btn-primary <?php if ($situacao == "2") { ?> ls-active<?php } ?>">INATIVOS</a> </div>
        <hr>
        <label class="ls-label col-md-12"> <b class="ls-label-text">Buscar funcionário</b>
          <input type="text" class="buscar-funcionario" alt="fonte-tabela"
            placeholder="Digite o nome ou parte do nome de um funcionário" />
        </label>
          ...
          <table class="ls-table fonte-tabela">
            <thead>
              <tr>
                ...
                <th class="ls-txt-center" width="150"></th>
              </tr>
            </thead>
            <?php
            $num = 1;
            ...
            <tbody>
              <?php do { ?>
                <tr>
                  ...
                  <td class="ls-txt-center"><?php isset($row_Funcionarios['funcao_nome']) ? print($row_Funcionarios['funcao_nome']) : print("Função não adicionada") ?></td>
                  <td class="ls-txt-center"><?php echo $row_Funcionarios['regime_nome']; ?></td>
                  <td class="ls-txt-center">

                    <div class="ls-group-btn ls-group-active ls-float-right">
                      <a title="Editar dados" href="funcionarios_editar.php?cod=<?php echo $row_Funcionarios['func_id']; ?>"
                        class="ls-btn ls-ico-pencil ls-btn-xs"></a>
                      <a title="Formação" href="titulacao_cadastrar.php?cod=<?php echo $row_Funcionarios['func_id']; ?>"
                        class="ls-btn ls-ico-attachment ls-btn-xs"></a>
                      <a title="Excluir" href="javascript:func()"
                        onclick="confirmacao('<?php echo $row_Funcionarios['func_id']; ?>')"
                        class="ls-btn ls-ico-remove ls-btn-xs ls-color-danger"></a>
                    </div>
  ...
  <script language="Javascript">
    function confirmacao(id) {
      var resposta = confirm("Deseja remover esse cadastro? Você não poderá recuperar os dados após a exclusão.");
