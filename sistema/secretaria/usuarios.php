...
mysql_select_db($database_SmecelNovo, $SmecelNovo);
$query_Usuarios = "
SELECT usu_id, usu_nome, usu_email, usu_senha, usu_tipo, usu_sec, usu_escola, usu_status, usu_cadastro, usu_contato, usu_cargo, usu_foto, escola_id, escola_nome,
CASE usu_tipo
...
WHEN 3 THEN 'COORDENADOR PEDAGOGICO'
WHEN 4 THEN 'SECRETARIO ESCOLAR'
WHEN 5 THEN 'AUXILIAR DE SECRETARIA'
WHEN 6 THEN 'GESTOR DE MATRICULAS'
WHEN 7 THEN 'OUTROS'
END AS usu_cargo_desc
FROM smc_usu
LEFT JOIN smc_escola ON escola_id = usu_escola 
WHERE usu_tipo <> '99' AND usu_sec = '$row_Secretaria[sec_id]' $tipo_query $status_query 
...
<main class="ls-main">
  <div class="container-fluid">
    ...
    <div class="ls-box ls-board-box">
      ...
      <label class="ls-label col-md-12"> <b class="ls-label-text">Buscar usuário</b>
        <input type="text" class="buscar-funcionario" alt="fonte-tabela" placeholder="Digite o nome ou parte do nome de um usuário" />
      </label>
      ...
      <table class="ls-table ls-sm-space fonte-tabela">
        <thead>
          <tr>
            <th width="50"></th>
            <th width="50"></th>
            <th width="260" >USUÁRIO / login</th>
            <th width="120" class="ls-txt-center">PAINEL</th>
            <th width="120" class="ls-txt-center">CARGO</th>
            <th width="100" class="ls-txt-center">CADASTRO</th>
            <th width="100" class="ls-txt-center">CONTATO</th>
...
?>
		...
		$num++;
		 ?></td>
            <td><img src="../../img/funcionarios/<?php echo $row_Usuarios['usu_foto']; ?>" width="100%" class="" style="border-radius:50%;"></td>
            <td><strong <?php if ($row_Usuarios['usu_contato']=="") { echo "class='ls-color-danger'"; } ?>><?php echo $row_Usuarios['usu_nome']; ?></strong><br><small><?php echo $row_Usuarios['usu_email']; ?></small><?php if ($row_Usuarios['usu_contato']=="") { ?><a href="#" class="ls-ico-help ls-float-right" data-trigger="hover" data-ls-module="popover" data-placement="left" data-content="Por medida de segurança, verifique se este usuário ainda é um usuário ativo no sistema. Caso contrário, clique em editar o usuário, marque como <strong class='ls-color-danger'>DESATIVADO</strong> e salve as informações." data-title="ATENÇÃO"></a><?php } ?></td>
            <td class="ls-txt-center"><?php echo $row_Usuarios['usu_tipo_desc']; ?></td>
            <td class="ls-txt-center"><?php echo $row_Usuarios['usu_cargo_desc']; ?></td>
            <td class="ls-txt-center"><?php echo date("d/m/Y", strtotime($row_Usuarios['usu_cadastro'])); ?></td>
            <td class="ls-txt-center"><?php echo $row_Usuarios['usu_contato']; ?></td>
            ...
            <td><a href="usuario_ver.php?usuario=<?php echo $row_Usuarios['usu_id']; ?>&hash=<?php echo md5($row_Usuarios['usu_id'].$row_Usuarios['usu_email']); ?>" class="ls-btn-primary ls-ico-eye ls-float-right"></a></td>
            <td><a href="usuarios_editar.php?codigo=<?php echo $row_Usuarios['usu_id']; ?>" class="ls-btn-primary ls-ico-edit-admin ls-float-right"></a></td>
          </tr>
