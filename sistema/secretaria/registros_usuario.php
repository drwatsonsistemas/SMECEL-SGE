...
WHERE escola_id_sec = '$row_UsuarioLogado[usu_sec]' AND registros_id_usuario = '$colname_usuario'
...
<main class="ls-main">
  <div class="container-fluid">
    <h1 class="ls-title-intro ls-ico-home">REGISTRO DE ATIVIDADES POR USUÁRIO</h1>
    <div class="ls-box ls-board-box"> 
    <!-- CONTEUDO -->

    <p>

        <strong>Usuário:</strong> <?php echo $row_Registros['usu_nome']; ?><br>
        <strong>Escola:</strong> <?php echo $row_Registros['escola_nome']; ?>
        <hr> 
  ...
  <div class="ls-group-btn">
    ...
    <a href="registros.php" class="ls-btn">Voltar</a>
    <table border="0">
      <tr>
        <td width="100"><?php if ($pageNum_Registros > 0) { // Show if not first page ?>
          <a href="<?php printf("%s?pageNum_Registros=%d%s", $currentPage, 0, $queryString_Registros); ?>" class="ls-btn">Primeiro</a>
          <?php } // Show if not first page ?></td>
        <td width="100"><?php if ($pageNum_Registros > 0) { // Show if not first page ?>
          <a href="<?php printf("%s?pageNum_Registros=%d%s", $currentPage, max(0, $pageNum_Registros - 1), $queryString_Registros); ?>" class="ls-btn">Anterior</a>
          <?php } // Show if not first page ?></td>
        <td width="100"><?php if ($pageNum_Registros < $totalPages_Registros) { // Show if not last page ?>
          <a href="<?php printf("%s?pageNum_Registros=%d%s", $currentPage, min($totalPages_Registros, $pageNum_Registros + 1), $queryString_Registros); ?>" class="ls-btn">Pr&oacute;ximo</a>
          <?php } // Show if not last page ?></td>
        <td width="100"><?php if ($pageNum_Registros < $totalPages_Registros) { // Show if not last page ?>
          <a href="<?php printf("%s?pageNum_Registros=%d%s", $currentPage, $totalPages_Registros, $queryString_Registros); ?>" class="ls-btn">&Uacute;ltimo</a>
          <?php } // Show if not last page ?></td>
      </tr>
    </table>
        ...
        <tbody>
          <?php do { ?>
            <tr>
              ...
              <td align="center"><strong><?php echo registros($row_Registros['registros_tipo']); ?></strong><br><?php echo $row_Registros['registros_complemento']; ?></td>
            </tr>
      ...
      Registros <?php echo ($startRow_Registros + 1) ?> a <?php echo min($startRow_Registros + $maxRows_Registros, $totalRows_Registros) ?> de <?php echo $totalRows_Registros ?>
      <?php } else { ?>
