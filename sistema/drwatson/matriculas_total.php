...
mysql_select_db($database_SmecelNovo, $SmecelNovo);
$query_AnoLetivo = "SELECT ano_letivo_id, ano_letivo_ano, ano_letivo_inicio, ano_letivo_fim, ano_letivo_aberto, ano_letivo_id_sec, ano_letivo_data_rematricula FROM smc_ano_letivo GROUP BY ano_letivo_ano DESC";
...
vinculo_aluno_internet, vinculo_aluno_multietapa, COUNT(*) AS total_matriculas, sec_id, sec_nome, sec_prefeitura, sec_faturar 
FROM smc_vinculo_aluno
INNER JOIN smc_sec ON sec_id = vinculo_aluno_id_sec
WHERE vinculo_aluno_ano_letivo = $ano
GROUP BY vinculo_aluno_id_sec
...
<script type="text/javascript">
      ...
      function drawChart() {
		...
		<?php
				...
				$date_ini = date("Y-m-d", strtotime("-15 days",strtotime($date_fim)));; //Data inicial
				$contaMatriculas = 0;
				...
				while (strtotime($date_ini) <= strtotime($date_fim)) {
					...
					FROM smc_vinculo_aluno
					WHERE vinculo_aluno_data = '$date_ini'";
					$Matriculas1 = mysql_query($query_Matriculas1, $SmecelNovo) or die(mysql_error());
					$row_Matriculas1 = mysql_fetch_assoc($Matriculas1);
					$totalRows_Matriculas1 = mysql_num_rows($Matriculas1);
						
				
				?>
				
				 ['<?php echo date("d/m", strtotime($date_ini)); ?>',  <?php echo $totalRows_Matriculas1; ?>],
				
				<?php 
				
				$date_ini = date ("Y-m-d", strtotime("+1 day", strtotime($date_ini)));
				$contaMatriculas1 = $contaMatriculas1 + $totalRows_Matriculas1;
				}		
...
<main class="ls-main">
  <div class="container-fluid">
    <h1 class="ls-title-intro ls-ico-home">MATRÍCULAS <?php echo $ano; ?></h1>
    
 <div class="ls-box-filter">
  <form action="matriculas_total.php" class="ls-form ls-form-inline" method="get">
    <input type="hidden" name="status" value="filtro">
    <label class="ls-label col-md-3 col-sm-12">
      <b class="ls-label-text">ANO</b>
      <div class="ls-custom-select">
        <select name="ano" id="select_period" class="ls-select">
        
			<?php do { ?>
             <option value="<?php echo $row_AnoLetivo['ano_letivo_ano']; ?>"><?php echo $row_AnoLetivo['ano_letivo_ano']; ?></option>
    ...
    <p class="ls-txt-right">DATA DE REFERÊNCIA <strong><?php echo date("d/m/Y"); ?></strong></p>

    <!-- CONTEUDO -->
    <table width="100%" class="ls-table">
      <thead>
      <tr>
        <th class="ls-txt-left">PREFEITURA</th>
        <th class="ls-txt-center" width="200">MATRÍCULAS</th>
      </tr>
      </thead>
      <tbody>
	  <?php 
	  ...
	  do { ?>
        <tr>
          <td class="ls-txt-left"><?php echo $row_Matriculas['sec_prefeitura']; ?></td>
          <td class="ls-txt-center"><?php echo $row_Matriculas['total_matriculas']; ?></td>
        </tr>
        <?php $total = $total + $row_Matriculas['total_matriculas']; ?>
        <?php } while ($row_Matriculas = mysql_fetch_assoc($Matriculas)); ?>
        <tr>
        	<td><h4 class="ls-txt-right"> TOTAL DE MATRÍCULAS </h4></td>
        	<td class="ls-txt-center"><h4 class="ls-txt-center"> <?php echo $total; ?> </h4></td>
        </tr>
    </tbody>
    </table>
...
<div class="ls-box ls-board-box">
            <div class="ls-box">
              <h5 class="ls-title-3 ls-txt-center"><?php echo $contaMatriculas1; ?> matrículas realizadas nos últimos 15 dias</h5>
