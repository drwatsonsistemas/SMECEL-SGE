...
vinculo_aluno_internet, vinculo_aluno_multietapa, MONTH(vinculo_aluno_data) AS mes_matricula, YEAR(vinculo_aluno_data) AS ano_matricula, COUNT(*) AS total_matriculas, sec_id, sec_nome, sec_prefeitura, sec_faturar 
FROM smc_vinculo_aluno
INNER JOIN smc_sec ON sec_id = vinculo_aluno_id_sec
WHERE YEAR(vinculo_aluno_data) = $ano AND sec_faturar = 'S'
GROUP BY mes_matricula
ORDER BY mes_matricula ASC";
...
<main class="ls-main">
 ...
 <div class="ls-box-filter">
  <form action="matriculas_mes.php" class="ls-form ls-form-inline" method="get">
    ...
    <table width="100%" class="ls-table">
      <thead>
      <tr>
        <th class="ls-txt-center" width="120">MÊS</th>
        <th class="ls-txt-center">ANO</th>
        <th class="ls-txt-center">MATRICULAS</th>
        <th class="ls-txt-center" width="120"></th>
      </tr>
      </thead>
      <tbody>
	  <?php 
	  ...
	  do { ?>
        <tr>
          <td><?php echo $row_Matriculas['mes_matricula']; ?></td>
          <td class="ls-txt-center"><?php echo $row_Matriculas['vinculo_aluno_ano_letivo']; ?></td>
          <td class="ls-txt-center"><?php echo $total_matriculas = $total_matriculas + $row_Matriculas['total_matriculas']; ?></td>
          <td class="ls-txt-right">R$ <?php echo number_format($total_matriculas * 0.2, 2, ',', '.'); ?></td>
        </tr>
        <?php $total = $total + $total_matriculas * 0.2; ?>
        <?php } while ($row_Matriculas = mysql_fetch_assoc($Matriculas)); ?>
        <tr>
        	<td colspan="4">    
            <h4 class="ls-txt-right">R$ <?php echo number_format($total, 2, ',', '.'); ?></h4></td>
        </tr>
    </tbody>
    </table>
    <!-- CONTEUDO -->    
