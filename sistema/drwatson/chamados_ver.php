...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
    ...
    if ($_POST['chamado_situacao'] == "F") {
        ...
        $updateGoTo = "chamados.php?reaberto";
    }
...
if ($row_Chamado['chamado_visualizado'] == 'N') {
    $updateSQL = "UPDATE smc_chamados SET chamado_visualizado = 'S' WHERE chamado_id = '$row_Chamado[chamado_id]'";
    mysql_select_db($database_SmecelNovo, $SmecelNovo);
    $Result1 = mysql_query($updateSQL, $SmecelNovo) or die(mysql_error());
}

// Marcar chamado como não visto
if ((isset($_POST["MM_mark_unseen"])) && ($_POST["MM_mark_unseen"] == "form_unseen")) {
    $updateSQL = sprintf(
        "UPDATE smc_chamados SET chamado_visualizado='N' WHERE chamado_id=%s",
        GetSQLValueString($_POST['chamado_id'], "text")
    );
    ...
    $updateGoTo = "chamados.php?marcado_nao_visto";
    if (isset($_SERVER['QUERY_STRING'])) {
        $updateGoTo .= (strpos($updateGoTo, '?')) ? "&" : "?";
        $updateGoTo .= $_SERVER['QUERY_STRING'];
    }
        ...
        <div class="container-fluid">
            ...
            <div class="ls-box ls-board-box">
                <header class="ls-info-header">
                    ...
                    <p class="ls-float-right ls-float-none-xs ls-small-info">
                        <?php echo $row_Chamado['chamado_data_abertura']; ?>
                    </p>
                </header>
            ...
            <?php if ($totalRows_Ticket > 0) { ?>
                ...
                do { ?>
                    <div class="ls-box ls-box-gray">
                        <p><strong>#<?php echo $resposta++; ?>         <?php echo $row_Ticket['usu_nome']; ?> respondeu em
                                <?php echo date("d/m/Y", strtotime($row_Ticket['ticket_data'])); ?>:</strong></p>
            ...
            <?php if ($row_Chamado['chamado_situacao'] == "F") { ?>
                <div class="ls-alert-info"><strong>Atenção:</strong>
                    Este chamado já foi finalizado. Caso necessite fazer uma nova interação, clique no botão abaixo para
                    reativar o chamado e escrever uma nova mensagem.
                </div>
            <?php } ?>

            <!-- Formulário para Nova Resposta e Voltar -->
            ...
            <!-- Formulário para Encerrar/Reabrir -->
            <form action="<?php echo $editFormAction; ?>" method="post" name="form_encerrar" id="form_encerrar"
                class="ls-form" style="display: inline;">
                <input type="submit"
                    value="<?php echo $row_Chamado['chamado_situacao'] == "A" ? "ENCERRAR ESTE CHAMADO" : "REABRIR ESTE CHAMADO"; ?>"
                    class="ls-btn-dark ls-float-right" />
                <input type="hidden" name="chamado_situacao"
                    value="<?php echo $row_Chamado['chamado_situacao'] == "A" ? "F" : "A"; ?>" />
                <input type="hidden" name="MM_update" value="form1" />
                <input type="hidden" name="chamado_id" value="<?php echo $row_Chamado['chamado_id']; ?>" />
            </form>

            <!-- Formulário para Marcar como Não Visto -->
            <?php if ($row_Chamado['chamado_situacao'] == "A") { ?>
            <form action="<?php echo $editFormAction; ?>" method="post" name="form_unseen" id="form_unseen"
                class="ls-form" style="display: inline;">
                <input type="submit" name="mark_unseen" value="MARCAR COMO NÃO VISTO"
                    class="ls-btn ls-btn-danger ls-float-right" style="margin-right: 10px;" />
                <input type="hidden" name="MM_mark_unseen" value="form_unseen" />
                <input type="hidden" name="chamado_id" value="<?php echo $row_Chamado['chamado_id']; ?>" />
            </form>
            <?php } ?>
            ...
            <div class="ls-modal" id="myAwesomeModal">
                <div class="ls-modal-box ls-modal-large">
                    <div class="ls-modal-header">
                        ...
                        <h4 class="ls-modal-title">MENSAGEM COMPLEMENTAR AO CHAMADO
                            #<?php echo $row_Chamado['chamado_numero']; ?></h4>
                    </div>
                    <div class="ls-modal-body" id="myModalBody">
                        ...
                        <form method="post" name="form1" action="<?php echo $editFormAction; ?>" class="ls-form">
                            ...
                            <input type="hidden" name="chamado_numero"
                                value="<?php echo $row_Chamado['chamado_numero']; ?>">
                            <input type="hidden" name="chamado_titulo"
                                value="<?php echo $row_Chamado['chamado_titulo']; ?>">
                            ...
                            <input type="hidden" name="ticket_id_chamado"
                                value="<?php echo $row_Chamado['chamado_id']; ?>">
                            <input type="hidden" name="ticket_id_usuario"
                                value="<?php echo $row_UsuarioLogado['usu_id']; ?>">
