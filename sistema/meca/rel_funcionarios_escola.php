...
mysql_select_db($database_SmecelNovo, $SmecelNovo);
$query_Vinculos = "SELECT vinculo_id, vinculo_id_escola, vinculo_id_funcionario, vinculo_id_funcao, COUNT(*) AS quant, escola_id, escola_nome FROM smc_vinculo INNER JOIN smc_escola ON escola_id = vinculo_id_escola GROUP BY vinculo_id_escola";
...
mysql_select_db($database_SmecelNovo, $SmecelNovo);
$query_Soma = "SELECT * FROM smc_vinculo";
$Soma = mysql_query($query_Soma, $SmecelNovo) or die(mysql_error());
...
<div class="small-12 columns">
  ...
  <table class="ls-table ls-no-hover ls-table-striped" width="100%">
    ...
    <?php do { ?>
      <tr>
        <td><?php echo $row_Vinculos['escola_nome']; ?></td>
        <td class="text-center"><a href="rel_funcionarios_lista_escola.php?cod=<?php echo $row_Vinculos['vinculo_id_escola']; ?>"><?php echo $row_Vinculos['quant']; ?></a></td>
      </tr>
      <?php } while ($row_Vinculos = mysql_fetch_assoc($Vinculos)); ?>
  </tbody>
  </table>
  <p class="text-right">Total: <?php echo $totalRows_Soma ?></p>
</div>
