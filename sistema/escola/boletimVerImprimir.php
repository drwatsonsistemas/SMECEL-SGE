    ...
    <thead>
      <tr height="30">
        ...
        <?php for ($p = 1; $p <= $row_criteriosAvaliativos['ca_qtd_periodos']; $p++) { ?>
          ...
          colspan="<?php echo $row_criteriosAvaliativos['ca_qtd_av_periodos'] + ($row_criteriosAvaliativos['ca_rec_paralela'] == "S" ? 1 : 1); ?>"><?php echo $p; ?>ª UNIDADE
          </th>
      ...
      <?php do { ?>
        <tr>
          ...
          <?php for ($p = 1; $p <= $row_criteriosAvaliativos['ca_qtd_periodos']; $p++) { ?>
            ...
            <?php if ($row_criteriosAvaliativos['ca_rec_paralela'] == "S") {
              mysql_select_db($database_SmecelNovo, $SmecelNovo);
              $query_notaRecPar = "SELECT nota_id, nota_id_matricula, nota_id_disciplina, nota_periodo, nota_num_avaliacao, nota_max, nota_min, nota_valor, nota_hash 
        FROM smc_nota 
        ...
        AND nota_num_avaliacao = '98'";
              ...
              // Exibe a nota de recuperação paralela
              ?>
            ...
            <td class="ls-txt-center ls-background-info" width="50">
              <strong>
                <?php
                // Calcula a média da unidade
                $mu = mediaUnidade(
                  $ru,
                  $row_criteriosAvaliativos['ca_arredonda_media'],
                  $row_criteriosAvaliativos['ca_aproxima_media'],
                  $row_criteriosAvaliativos['ca_media_min_periodo'],
                  $row_criteriosAvaliativos['ca_calculo_media_periodo'],
                  $row_criteriosAvaliativos['ca_qtd_av_periodos'],
                  $row_criteriosAvaliativos['ca_digitos']
                );
                //echo $mu;
                $tmu = $tmu + $mu;
                ?>
