...
mysql_select_db($database_SmecelNovo, $SmecelNovo);
$query_Criterios = "SELECT ca_id, ca_id_secretaria, ca_descricao, ca_qtd_periodos, ca_qtd_av_periodos, ca_nota_min_av, ca_nota_max_av, ca_calculo_media_periodo, ca_media_min_periodo, ca_arredonda_media, ca_aproxima_media, ca_min_pontos_aprovacao_final, ca_min_media_aprovacao_final, ca_nota_min_recuperacao_final, ca_detalhes, ca_rec_paralela FROM smc_criterios_avaliativos WHERE ca_id = '$row_AlterarStatus[matriz_criterio_avaliativo]'";
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
	...
	if ($row_UsuLogado['usu_insert']=="N") {
		die(header(sprintf("Location: vinculoAlunoExibirTurma.php?permissao")));
	}
...
//Loop disciplinas
    for ($p = 1; $p <= $numPeriodos; $p++) {
     ...
     for ($a = 1; $a <= $numAvaliacoes; $a++) {
      ...
      mysql_select_db($database_SmecelNovo, $SmecelNovo);
      $query_nota = "
      SELECT nota_id, nota_id_matricula, nota_id_disciplina, nota_periodo, nota_num_avaliacao, nota_max, nota_min, nota_valor, nota_hash FROM smc_nota 
      WHERE nota_id_matricula = '$idVinculo' AND nota_id_disciplina = '$row_disciplinas[matriz_disciplina_id_disciplina]' AND nota_periodo = '$p' AND nota_num_avaliacao = '$a'";
    ...
    if ($row_Criterios['ca_rec_paralela']=="S") {
      ...
      $query_notaRecPar = "
      SELECT nota_id, nota_id_matricula, nota_id_disciplina, nota_periodo, nota_num_avaliacao, nota_max, nota_min, nota_valor, nota_hash FROM smc_nota 
      WHERE nota_id_matricula = '$idVinculo' AND nota_id_disciplina = $idDisciplina AND nota_periodo = '$p' AND nota_num_avaliacao = '98'";
...
$updateGoTo = "boletimVer.php?boletimcadastrado";
if (isset($_SERVER['QUERY_STRING'])) {
  $updateGoTo .= (strpos($updateGoTo, '?')) ? "&" : "?";
  $updateGoTo .= $_SERVER['QUERY_STRING'];
}
...
<body>
  ...
  <main class="ls-main ">
    <div class="container-fluid">
      ...
      <h1 class="ls-title-intro ls-ico-home">CORRIGIR BOLETIM</h1>
      <!-- CONTEÚDO -->
      ...
      <form method="post" name="form1" action="<?php echo $editFormAction; ?>" class="ls-form-horizontal row" data-ls-module="form">
        ...
        <div class="ls-actions-btn">
          <input type="submit" value="ACRESCENTAR NOTAS NO BOLETIM" class="ls-btn-primary">
