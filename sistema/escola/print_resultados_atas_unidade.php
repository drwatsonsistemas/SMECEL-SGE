...
if ($row_Turmas['etapa_id_filtro'] == 1) {

	die(header("Location: print_resultados_atas_unidade_ed_inf.php?ct=$codTurma&ano=$anoLetivo&unidade=$codUnidade"));
						...
						<?php if (($row_AlunoBoletim['vinculo_aluno_situacao'] <> "1") || ($row_AlunoBoletim['vinculo_aluno_reprovado_faltas'] == "S")) { ?>
							...
							<?php } else { ?>
								<td align="center"
									...
									<?php echo $row_AlunoBoletim['vinculo_aluno_situacao_nome']; ?></td>
							<?php } ?>
						<?php } else { ?>
							<?php do { ?>
								...
								if ($row_disciplinasMatriz['matriz_disciplina_reprova'] == "S" && $row_CriteriosAvaliativos['ca_questionario_conceitos'] == "N") {
									...
									if ($row_CriteriosAvaliativos['ca_rec_paralela'] == "S") {
										...
										mysql_select_db($database_SmecelNovo, $SmecelNovo);
										$query_notaRecPar = "SELECT nota_id, nota_id_matricula, nota_id_disciplina, nota_periodo, nota_num_avaliacao, nota_max, nota_min, nota_valor, nota_hash FROM smc_nota WHERE nota_id_matricula = '$row_AlunoBoletim[vinculo_aluno_id]' AND nota_id_disciplina = '$row_disciplinasMatriz[disciplina_id]' AND nota_periodo = '$codUnidade' AND nota_num_avaliacao = '98'";
							...
							";
									...
									WHERE conc_ef_id_quest = '$row_Acompanhamento[quest_conc_id]' AND conc_ef_id_matr = '$row_AlunoBoletim[vinculo_aluno_id]' AND conc_ef_periodo = '$codUnidade' AND conceito_itens_id_conceito = '$row_CriteriosAvaliativos[ca_grupo_conceito]'
									";
