...
<head>
  ...
  <title>ATA DE RESULTADOS FINAIS POR PERÍODO -
    <?php echo $row_AlunoBoletim['turma_nome'] . " - " . $row_EscolaLogada['escola_nome'] ?>
  </title>
  <meta charset="utf-8">
...
<body onload="window.print()">
  <div class="container-fluid">
    <?php if ($row_CriteriosAvaliativos['ca_questionario_conceitos'] == "S") { ?>
      <div class="ls-box1 ls-txt-center ls-sm-space">
        <div class="ls-btn-group">
          <a href="?ct=<?php echo $codTurma; ?>&ano=<?php echo $anoLetivo; ?>&tipo=legenda"
            class="ls-btn <?php echo ($tipo == 'legenda') ? 'ls-btn-primary' : 'ls-btn-default'; ?>">LEGENDAS</a>
          <a href="?ct=<?php echo $codTurma; ?>&ano=<?php echo $anoLetivo; ?>&tipo=porcentagem"
            class="ls-btn <?php echo ($tipo == 'porcentagem') ? 'ls-btn-primary' : 'ls-btn-default'; ?>">PORCENTAGEM</a>
        </div>
      </div>
    <?php } ?>
    <div style="page-break-inside: avoid;" class="ls-box1 ls-txt-center" align="center"> <br>
      ...
      <p>
      <h2 class="ls-txt-center">ATA DE RESULTADOS FINAIS POR PERÍODO -
        <strong><?php echo $row_AlunoBoletim['turma_nome'] ?></strong>
      </h2>
      </p>
            ...
            <tr>
              <td class="ls-txt-left">
                ...
                <span class="ls-tag-info">
                  <?php echo $row_GrupoConceitos['conceito_itens_legenda']; ?>:
                  <?php echo $row_GrupoConceitos['conceito_itens_descricao']; ?>
                </span>
              </td>
            </tr>
      ...
      <table width="100%" class="ls-sm-space ls-table-striped bordasimples">
        <tr>
          ...
          <?php for ($i = 1; $i <= $row_CriteriosAvaliativos['ca_qtd_periodos']; $i++) { ?>
            <th style="background-color: #90ee90" colspan="<?php echo $totalRows_disciplinasMatrizCab + 1 ?>">
              <?php echo $i . "° Período" ?>
            </th>
          <?php } ?>
        </tr>
        ...
        <?php do { ?>
          ...
          <tr>
            ...
            <td width="25%" align="left">
              <?php echo $row_AlunoBoletim['aluno_nome_social'] != "" ? $row_AlunoBoletim["aluno_nome_social"] : $row_AlunoBoletim["aluno_nome"]; ?>
              <?php if ($row_AlunoBoletim['vinculo_aluno_conselho'] == "S") { ?>
                *
              <?php } ?>
              <?php if ($row_AlunoBoletim['vinculo_aluno_situacao_nome'] != "MATRICULADO") { ?>
                <span
                  style="font-size: 10px;color:#ff6961 ">(<?php echo $row_AlunoBoletim['vinculo_aluno_situacao_nome'] ?>)</span>
              <?php } ?>
              ...
              <?php foreach ($disciplinas as $disciplina): ?>
                <div style="display:none">
                  ...
                  for ($p = 1; $p <= $row_CriteriosAvaliativos['ca_qtd_periodos']; $p++) {
                    ...
                    for ($a = 1; $a <= $row_CriteriosAvaliativos['ca_qtd_av_periodos']; $a++) {
                      ...
                      $query_nota = "SELECT nota_id, nota_id_matricula, nota_id_disciplina, nota_periodo, nota_num_avaliacao, nota_max, nota_min, nota_valor, nota_hash FROM smc_nota WHERE nota_id_matricula = '$row_AlunoBoletim[vinculo_aluno_id]' AND nota_id_disciplina = '$disciplina[disciplina_id]' AND nota_periodo = '$i' AND nota_num_avaliacao = '$a'";
                      ...
                      $ru = $ru + number_format($row_nota['nota_valor'], $row_CriteriosAvaliativos['ca_digitos']);
                    }
                    $mu = mediaUnidade($ru, $row_CriteriosAvaliativos['ca_arredonda_media'], $row_CriteriosAvaliativos['ca_aproxima_media'], $row_CriteriosAvaliativos['ca_media_min_periodo'], $row_CriteriosAvaliativos['ca_calculo_media_periodo'], $row_CriteriosAvaliativos['ca_qtd_av_periodos'], $row_CriteriosAvaliativos['ca_digitos']);
                    $tmu = $tmu + $mu;
                  }


                  $tp = totalPontos($tmu, $row_CriteriosAvaliativos['ca_digitos']);
                ...
                <td>
                  ...
                  if ($row_AlunoBoletim['vinculo_aluno_situacao'] == 1) {
                    if ($row_disciplinasMatriz['matriz_disciplina_reprova'] == "S" && $row_CriteriosAvaliativos['ca_questionario_conceitos'] == "N") {

                      if ($row_CriteriosAvaliativos['ca_rec_paralela'] == "S") {
                        // Busca nota da recuperação paralela
                        mysql_select_db($database_SmecelNovo, $SmecelNovo);
                        $query_notaRecPar = "SELECT nota_id, nota_id_matricula, nota_id_disciplina, nota_periodo, nota_num_avaliacao, nota_max, nota_min, nota_valor, nota_hash FROM smc_nota WHERE nota_id_matricula = '$row_AlunoBoletim[vinculo_aluno_id]' AND nota_id_disciplina = '$disciplina[disciplina_id]' AND nota_periodo = '$i' AND nota_num_avaliacao = '98'";
                        ...
                        if ($row_notaRecPar && $row_notaRecPar['nota_valor'] !== null) {
                          if ($row_CriteriosAvaliativos['ca_calculo_media_periodo'] == "MEDIA") {
                            $notaRecPar = $row_notaRecPar['nota_valor'] * $row_CriteriosAvaliativos['ca_qtd_av_periodos'] / $row_CriteriosAvaliativos['ca_qtd_periodos'];
                          } else {
            ...
            //echo 6.0;
                    ...
                    WHERE quest_conc_id_matriz = '$row_AlunoBoletim[turma_matriz_id]' AND quest_conc_id_comp = '$disciplina[disciplina_id]'
                    ...
                    ";
                      ...
                      do {
                        $acompanhamentos[] = $row_Acompanhamento;
                      } while ($row_Acompanhamento = mysql_fetch_assoc($Acompanhamento));

                      for ($x = 1; $x <= $row_CriteriosAvaliativos['ca_qtd_periodos']; $x++) {
                        $tot[$x] = 0;
                      }
                    ...
                    WHERE conc_ef_id_quest = '$acompanhamento[quest_conc_id]' AND conc_ef_id_matr = '$row_AlunoBoletim[vinculo_aluno_id]' AND conc_ef_periodo = '$i' AND conceito_itens_id_conceito = '$row_CriteriosAvaliativos[ca_grupo_conceito]'
                    ";
                        ...
                        $faixas = array();
                        $inicio = 0;
                        $perc = number_format(100 / mysql_num_rows($GrupoConceitos), 0);
                        $parc = $perc;
                      }
                      if (count(array_filter($tot, 'filtroZero')) == count($tot)) {
                        ...
                        if ($tipo == "porcentagem") {
                          ...
                          // Lógica para exibir legendas
                          $porcentagem = number_format((($tot[$i] / ($row_CriteriosAvaliativos['ca_qtd_periodos'] * $totalRows_Acompanhamento)) * 100), 0);

                          // Buscar as legendas do banco
                          mysql_select_db($database_SmecelNovo, $SmecelNovo);
                          $query_GrupoConceitos = "
                                  SELECT conceito_itens_id, conceito_itens_id_conceito, conceito_itens_descricao, 
                          ...
                          // Array para armazenar as faixas
                          $faixas = array();
                          $inicio = 0;
                          $perc = number_format(100 / mysql_num_rows($GrupoConceitos), 0);
                          $parc = $perc;

                          while ($row = mysql_fetch_assoc($GrupoConceitos)) {
                            $faixas[] = array(
                              'inicio' => $inicio,
                              'fim' => min($parc, 100),
                              'legenda' => $row['conceito_itens_legenda']
                            );
                            $inicio = min($parc + 1, 100);
                            $parc = min($parc + $perc, 100);
                          }

                          // Encontrar a legenda correspondente
                          $legenda_encontrada = "-";
                          foreach ($faixas as $faixa) {
                            if ($porcentagem >= $faixa['inicio'] && $porcentagem <= $faixa['fim']) {
                              $legenda_encontrada = $faixa['legenda'];
                              break;
                            }
                          }
                          if ($porcentagem == 100) {
                            $legenda_encontrada = end($faixas)['legenda'];
                          }
                          echo $legenda_encontrada;
                        }
                      }
                    }
                    $mu = 0;
