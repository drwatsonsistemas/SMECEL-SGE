...
vinculo_aluno_situacao, vinculo_aluno_datatransferencia, vinculo_aluno_da_casa, vinculo_aluno_historico_transferencia, vinculo_aluno_conselho_parecer,vinculo_aluno_resultado_final,
vinculo_aluno_vacina_atualizada, aluno_id, aluno_nome, aluno_nome_social, aluno_cod_inep, aluno_nascimento, aluno_foto, aluno_filiacao1, aluno_hash, aluno_observacao, turma_id, turma_nome, turma_turno,
CASE turma_turno
...
<body onLoad="print()">
  ...
  <table width="100%">
    <tr>
      <td width="100px">
        ...
        <?php if ($row_EscolaLogada['escola_logo'] <> "") { ?><img
            ...
            width="100%" /><?php } else { ?><img src="../../img/brasao_republica.png" alt="" width="100%" /><?php } ?>
    ...
    <tr>
      <td colspan="3"><br>
        <h2 class="ls-txt-center">FICHA INDIVIDUAL DO ALUNO - <?= $row_matricula['vinculo_aluno_ano_letivo'] ?></h2>
      </td>
    </tr>
          ...
          <?php for ($p = 1; $p <= $row_criteriosAvaliativos['ca_qtd_periodos']; $p++) { ?>
            ...
            if ($row_criteriosAvaliativos['ca_rec_paralela'] == "S") {
              ...
              <th class="ls-txt-center" width="50">
                <?php echo exibeTraco($row_notaRecPar['nota_valor'], $row_criteriosAvaliativos['ca_nota_min_av'], $row_criteriosAvaliativos['ca_digitos']); ?></th>
            <?php } ?>
  ...
  <small>
    AV(*): Avaliação | RP: Recuperação Paralela | RU: Resultado da Unidade | TP: Total de pontos | MC: Média do curso |
    AF: Avaliação final | RF: Resultado final | ** Componente não reprova
  </small>
  ...
  <table class="ls-table1 ls-sm-space bordasimples" width="100%">
    <tr>
      <td><br>Resultado final: <?php 
      if($row_matricula['vinculo_aluno_resultado_final'] == 1){
        echo 'APROVADO';
      }else if($row_matricula['vinculo_aluno_resultado_final'] == 2){
        echo 'REPROVADO';
      }else{
        echo '<i>PARA EXIBIR INFORMAÇÕES DE APROVAÇÃO, É NECESSÁRIO CONSOLIDAR A TURMA.</i>';
      }
      ?> <br><br></td>
      <td><br>Visto em: ______/_______/_____________<br><br></td>
    </tr>
    ...
    <?php } ?>
    <tr>
      <td colspan="2"><br>Transferência entregue em: ______/_______/_____________<br><br></td>
    </tr>
    <tr>
      <td colspan="2"><br>Assinatura do responsável:
