...
$queryDisciplinas = "SELECT matriz_disciplina_id_disciplina FROM smc_matriz_disciplinas WHERE matriz_disciplina_id_matriz = '{$row_AlterarStatus['matriz_id']}'";
$disciplinas = mysql_query($queryDisciplinas, $SmecelNovo);

// Verifica se tem recuperação paralela
$temRecParalela = $row_Criterios['ca_rec_paralela'] == 'S';

// Cria os registros de notas para cada disciplina
while ($row_disciplinas = mysql_fetch_assoc($disciplinas)) {
    $idDisciplina = $row_disciplinas['matriz_disciplina_id_disciplina'];
    
    // Cria registros para cada período e avaliação
    for ($p = 1; $p <= $row_Criterios['ca_qtd_periodos']; $p++) {
        for ($a = 1; $a <= $row_Criterios['ca_qtd_av_periodos']; $a++) {
