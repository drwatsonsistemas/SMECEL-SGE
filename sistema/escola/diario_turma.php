...
<body>
  ...
  <main class="ls-main ">
    <div class="container-fluid">
      ...
      <h1 class="ls-title-intro ls-ico-home">DIÁRIO DE CLASSE | Ano Letivo <?php echo $anoLetivo; ?></h1>
      <!-- CONTEÚDO -->
      ...
      <div class="ls-box1">
        ...
        <a href="diarios_preenchidos.php?ano=<?php echo $anoLetivo; ?>" class="ls-btn ls-float-left"><span
            class="ls-ico-chevron-left ls-ico-right"></span>&nbsp;VOLTAR</a>
        <a href="print_diario_turma.php?turma=<?php echo $colname_Turma; ?>&ano=<?php echo $anoLetivo; ?>"
          target="_blank" class="ls-btn-primary"><span
            class="ls-ico-paint-format ls-ico-right"></span>&nbsp;IMPRIMIR</a>
...
SELECT 
    plano_aula_id, 
    plano_aula_id_turma, 
    plano_aula_id_disciplina, 
    plano_aula_id_professor, 
    plano_aula_data, 
    plano_aula_data_cadastro, 
    plano_aula_texto, 
    plano_aula_conteudo, 
    plano_aula_atividade, 
    plano_aula_atividade_resposta_obrigatoria, 
    plano_aula_atividade_resposta_obrigatoria_data_limite, 
    plano_aula_video, 
    plano_aula_google_form, 
    plano_aula_google_form_tempo, 
    plano_aula_publicado, 
    plano_aula_hash, 
    func_id, 
    func_nome 
FROM 
    smc_plano_aula
INNER JOIN
    smc_func ON func_id = plano_aula_id_professor
WHERE
    plano_aula_id_disciplina = '$row_Disciplinas[disciplina_id]' 
    AND plano_aula_id_turma = '$row_Turma[turma_id]'
ORDER BY 
    plano_aula_data
";
        ...
        <?php if ($totalRows_Aulas > 0) { ?>
          ...
          <div class="ls-box">
            <p>Professores(as):
              ...
              <strong>
                <?php do { ?>
                  <?php echo $row_AulasCab['func_nome']; ?> (<?php echo $row_AulasCab['func_id']; ?>)<br>
                <?php } while ($row_AulasCab = mysql_fetch_assoc($AulasCab)); ?>
              </strong>
            </p>
          ...
          <table class="ls-table ls-sm-space">
            <tr>
              ...
              <th width="80" class="ls-txt-center">COD</th>
              <th class="ls-txt-left">CONTEÚDO</th>
              <th class="ls-txt-center">PROFESSOR(A)</th>
              <th width="110" class="ls-txt-right"></th>
            </tr>
            ...
            <?php do { ?>
              <tr>
                ...
                <td class="ls-txt-center"><?php echo $row_Aulas['func_nome']; ?></td>
                <td>
                  <button class="delete-btn ls-ico-remove" style="color:red; background:none; border:none; cursor:pointer;"
                    data-id="<?php echo $row_Aulas['plano_aula_hash']; ?>">
                    Excluir
                  </button>
                </td>
              </tr>
          ...
          <?php do { ?>
            ...
            <p class="ls-txt-center">
              ...
              Assinatura do(a) professor(a)<br>
              <?php echo $row_AulasAss['func_nome']; ?> (<?php echo $row_AulasAss['func_id']; ?>)
            </p>
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      deleteButtons.forEach(button => {
        button.addEventListener('click', function () {
          const aulaId = this.dataset.id; // Obtém o ID do botão clicado

          if (confirm('Tem certeza que deseja excluir esta aula?')) {
            // Envia a requisição POST para exclusão
            fetch('deletar_aulas_diario.php', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json'
              },
              body: JSON.stringify({ aula: aulaId })
            })
              ...
              .then(data => {
                if (data.success) {
                  ...
                  alert('Erro ao excluir a aula: ' + data.message);
                }
              })
              .catch(error => {
