                ...
                <?php for ($p = 1; $p <= $row_Criterios['ca_qtd_periodos']; $p++) { ?>
                  ...
                  <?php
                    ...
                    if ($row_Criterios['ca_rec_paralela'] == "S") {
                      ...
                      mysql_select_db($database_SmecelNovo, $SmecelNovo);
                      $query_notaRecPar = "SELECT nota_id, nota_id_matricula, nota_id_disciplina, nota_periodo, nota_num_avaliacao, nota_max, nota_min, nota_valor, nota_hash FROM smc_nota WHERE nota_id_matricula = '$row_Alunos[vinculo_aluno_id]' AND nota_id_disciplina = $row_Disciplina[disciplina_id] AND nota_periodo = '$p' AND nota_num_avaliacao = '98'";
                      ...
                      if ($row_notaRecPar && $row_notaRecPar['nota_valor'] !== null) {
                        if ($row_Criterios['ca_calculo_media_periodo'] == "MEDIA") {
                          $ru = $row_notaRecPar['nota_valor'] * $row_Criterios['ca_qtd_av_periodos'];
                        } else {
                      ...
                      <td class="ls-txt-center" width="50">
                        <?php echo exibeTraco($row_notaRecPar['nota_valor'], $row_Criterios['ca_nota_min_av'], $row_Criterios['ca_digitos']); ?>
                      </td>
                    <?php } ?>
