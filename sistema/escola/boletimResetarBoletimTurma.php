...
if ($row_Criterios['ca_questionario_conceitos'] == "S") {
  ...
  header("Location:boletimResetarBoletimConceitoEF.php?turma=$colname_Turma");
  die;
      ...
      <?php if ($totalRows_Alunos > 0) { ?>
        ...
        <table class="ls-table">
          ...
          <?php do { ?>
            ...
            $deleteSQL = sprintf(
              "DELETE FROM smc_nota WHERE nota_id_matricula = %s",
            ...
            $updateSQL = sprintf(
              "UPDATE smc_vinculo_aluno SET vinculo_aluno_boletim='0' WHERE vinculo_aluno_id=%s",
              GetSQLValueString($idVinculo, "int")
            );
