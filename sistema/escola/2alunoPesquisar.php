...
if($inepBusca != ""){ $inep_no_where = "AND aluno_cod_inep LIKE '%".$inepBusca."%'"; } else { $inep_no_where = ""; }
if($cpfBusca != ""){ $cpf_no_where = "AND aluno_cpf LIKE '%".$cpfBusca."%'"; } else { $cpf_no_where = ""; }
//if($nomeBusca != ""){ $nome_no_where = "AND aluno_nome LIKE '".$nomeBusca."%'"; } else { $nome_no_where = ""; }
if($nomeBusca != ""){ $nome_no_where = "AND MATCH (aluno_nome) AGAINST ('".$nomeBusca."  IN BOOLEAN MODE')"; } else { $nome_no_where = ""; }
if($nascimentoBusca != ""){ $nascimento_no_where = "AND aluno_nascimento LIKE '%".$nascimentoBusca."%'"; } else { $nascimento_no_where = ""; }
//if($maeBusca != ""){ $mae_no_where = "AND (aluno_filiacao1 LIKE '%".$maeBusca."%' OR aluno_filiacao2 LIKE '%".$maeBusca."%')"; } else { $mae_no_where = ""; }
if($maeBusca != ""){ $mae_no_where = "AND (MATCH (aluno_filiacao1) AGAINST ('".$maeBusca."') OR MATCH (aluno_filiacao2) AGAINST ('".$maeBusca."'))"; } else { $mae_no_where = ""; }
...
$link = "vinculoAlunoCadastrarRematricula.php";
...
<link rel="manifest" href="https://www.smecel.com.br/site.webmanifest">
      ...
      <div class="container-fluid">
        ...
        <h1 class="ls-title-intro ls-ico-users">Pesquisar aluno <?php echo $tituloRematricula; ?></h1>
              ...
              <?php } ?>
				...
				<p>Pesquise o aluno para depois vinculá-lo em uma turma.
                <a href="#" class="ls-ico-help ls-float-right" data-ls-module="modal" data-target="#dicas"></a>
                </p>
        ...
        <form action="" method="post" class="ls-form ls-form-horizontal row">
          <fieldset>
            ...
            <input type="text" class="ls-field-sm cpf" name="cpf" id="cpf" onblur="javascript: validarCPF(this);" onkeypress="javascript: mascara(this, cpf_mask);" placeholder="CPF do aluno">
            </label>
			
		    <label class="ls-label col-md-6">
            <b class="ls-label-text">INEP</b>
            <input type="text" class="ls-field-sm inep" name="inep" id="inep" placeholder="Código INEP">
            </label>
            ...
            <input type="text" class="ls-field-sm" name="nome" id="nome" pattern=".{4,}" title="Informe no mínimo 4 caracteres" placeholder="Nome completo ou parte" required>
            </label>
				
            <label class="ls-label col-md-2">
            <b class="ls-label-text">NASCIMENTO</b>
            <input type="text" class="ls-field-sm date" name="nascimento" id="nascimento" placeholder="Data de nascimento" required>
            </label>
				
            <label class="ls-label col-md-5">
            <b class="ls-label-text">NOME DA FILIAÇÃO 1 OU FILIAÇÃO 2</b>
            <input type="text" class="ls-field-sm" name="mae" id="mae" placeholder="Nome da filiação 1 ou filiação 2" required>
            </label>
  ...
  <table class="ls-table ls-table-striped ls-sm-space">
    <thead>
      <tr>
        <th width="120px" class="ls-txt-center">INEP</th>
        <th>RESULTADO</th>
    ...
    <tbody>
      <?php do { ?>
        <tr>
          <td class="ls-txt-center"><?php echo $row_buscaFuncionario['aluno_cod_inep']; ?></td>
		  
		  <td> 
		  <p><strong>ALUNO(A):</strong> <?php echo $row_buscaFuncionario['aluno_nome']; ?></p>
		  ...
		  <p><strong>MUNICÍPIO DE NASCIMENTO: </strong><?php echo $row_buscaFuncionario['municipio_nome']; ?> - <?php echo $row_buscaFuncionario['municipio_sigla_uf']; ?></p>
		  
		  </td>
		  <td class="ls-txt-center"><a href="<?php echo $link; ?>?c=<?php echo $row_buscaFuncionario['aluno_hash']; ?>" class="ls-ico-user-add"> <?php echo $linkNome; ?></a></td>
        </tr>
  ...
  Nenhum resultado. <a class="" href="alunoCadastrar.php">Cadastrar aluno</a>
  </div>
  <hr>
  <p>
  <a class="ls-btn-primary ls-btn-lg ls-ico-windows" href="alunoCadastrar.php">Cadastrar aluno</a>
  </p>
  <hr>
  <hr>
  <?php } ?>
  ...
  <div class="ls-modal-box">
    ...
    <div class="ls-modal-body" id="myModalBody">
      <p>
                ...
                - Para fazer a busca pelo nome do aluno, será necessário informar a data de nascimento e a filiação (nome do pai ou da mãe) do aluno;<br>
                - Você poderá informar apenas o primeiro nome do aluno. Ex.: JOÃO PEREIRA DOS SANTOS NETO (pode ser informado apenas JOAO);<br>
                - É possível informar o primeiro nome da filiação, como também o sobrenome. Ex.: No caso da mãe do aluno se chamar MARIA DE JESUS SANTOS, você poderá digitar apenas MARIA, ou então SANTOS;<br>
                - Caso nenhum aluno seja encontrado após uma cuidadosa pesquisa, clique no botão "Cadastrar aluno", logo abaixo do campo de resultado da pesquisa;
