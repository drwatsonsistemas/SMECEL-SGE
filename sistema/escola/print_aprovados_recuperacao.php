...
<title>RESULTADO PARCIAL - ANO LETIVO <?php echo $row_Turmas['turma_ano_letivo']; ?> | SMECEL - Sistema de Gestão Escolar</title>
<meta charset="utf-8">
...
?>
  ...
  <?php 
	...
	vinculo_aluno_datatransferencia, aluno_id, aluno_nome, aluno_nome_social,aluno_nascimento 
	FROM smc_vinculo_aluno 
	INNER JOIN smc_aluno ON aluno_id = vinculo_aluno_id_aluno 
	WHERE vinculo_aluno_id_escola = $row_EscolaLogada[escola_id] AND vinculo_aluno_id_turma = $row_Turmas[turma_id] AND vinculo_aluno_ano_letivo = $row_AnoLetivo[ano_letivo_ano] ORDER BY aluno_nome ASC";
  ...
  <div style="page-break-inside: avoid;">
    <div class="ls-box ls-txt-center">
      ...
      <p>
      <h2 class="ls-txt-center">RESULTADO PARCIAL - ANO LETIVO <?php echo $row_Turmas['turma_ano_letivo']; ?><br>
        <?php echo $row_Turmas['turma_nome']; ?>-<?php echo $row_Turmas['turma_turno_nome']; ?></h2>
      </p>
      <p>
      
      <div style="text-align:justify; line-height:150%;"> Segue relação de alunos aprovados e os que necessitam de reorientação por disciplina na turma: <br>
      </div>
      </p>
      <table width="100%" class="ls-sm-space ls-table-striped bordasimples">
        <thead>
          <tr>
            <th>Nº</th>
            <th width="200">NOME DO(A) ALUNO(A)</th>
          ...
          <?php do { ?>
            <tr>
              <td align="center" width="25px"><?php 
					...
					$contaAlunos++;		
					?></td>
              <td width="300"><?php echo $row_Matriculas['aluno_nome_social'] != "" ? $row_Matriculas["aluno_nome_social"] : $row_Matriculas["aluno_nome"]; ?></td>
              <td width="100" align="center"><?php echo inverteData($row_Matriculas['aluno_nascimento']); ?></td>
              <td><?php
					  mysql_select_db($database_SmecelNovo, $SmecelNovo);
				...
				?>
                    <?php do { ?>
                      <td align="center" width="11%" style="border:0px solid #808080; border-right:1px solid #808080;"><strong>
                        <div style="display:none"> <?php echo $row_Notas['disciplina_nome']; ?>
				...
				?>
                  ...
                  <?php } else { ?>
                  <tr>
                    <td align="center" style="border:0px solid #808080; border-right:1px solid #808080; letter-spacing:15px;"><small><?php echo $row_Matriculas['vinculo_aluno_situacao_nome']; ?></small></td>
                  </tr>
                  <?php } ?>
                </table></td>
              <td width="300"></td>
      ...
      <p>
      <?php
	  $aprovadosTotal = $aprovadosTotal + $aprovados;
	  $reprovadosTotal = $reprovadosTotal + $reprovados;
	  ?>
      <div style="text-align:justify; line-height:150%;"> Aprovados: <?php echo $aprovados - $reprovados; ?> <br>
        Reorientação: <?php echo $reprovados; ?> <br>
        <hr>
        <small></i>Relatório impresso em <?php echo date('d/m/Y'); ?> às <?php echo date('H:i:s'); ?>. <br>
        SMECEL - Sistema de Gestão Escolar</i></small> <br>
      </div>
      </p>
    </div>
  </div>
...
TOTAL DE APROVADOS: <?php echo $aprovadosTotal - $reprovadosTotal; ?><br>
TOTAL DE REPROVADOS: <?php echo $reprovadosTotal; ?>
