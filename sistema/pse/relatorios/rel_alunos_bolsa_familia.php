...
vinculo_aluno_verificacao, vinculo_aluno_boletim, vinculo_aluno_situacao, vinculo_aluno_datatransferencia, vinculo_aluno_vacina_atualizada,
aluno_id, aluno_nome, aluno_nascimento, aluno_filiacao1, aluno_nis, aluno_sus, aluno_cpf, aluno_recebe_bolsa_familia,
turma_id, turma_nome,
CASE aluno_recebe_bolsa_familia
WHEN '1' THEN 'SIM'
WHEN '0' THEN ''
END AS aluno_recebe_bolsa_familia_nome
...
<h4 align="center">RELAÇÃO DE ALUNOS - SITUAÇÃO VACINAL</h4>
...
<?php if ($totalRows_Matricula > 0) { // Show if recordset not empty ?>
  <table class="bordasimples" width="100%" style="font-size:11px;">
    <thead>
      <tr>
        ...
        <th class="center">BOLSA<br>FAMÍLIA</th>
        </tr>
    </thead>
    <tbody>
      <?php
  ...
  ?>
	  ...
	  $cod++; 
	   ?>
          ...
          <td class="center"><?php echo $row_Matricula['aluno_recebe_bolsa_familia_nome']; ?></td>
        </tr>
		
		<?php 
		
		 if ($row_Matricula['aluno_recebe_bolsa_familia']=="1") {
			 $bolsa++;
		 } 
  ...
  <p class="right">
  Total de alunos: <?php echo $totalRows_Matricula; ?><br>
  Beneficiários do Bolsa Família: <?php echo $bolsa; ?>; <br>
  Não recebem: <?php echo $totalRows_Matricula - $bolsa; ?>
  </p>
  <?php } else { ?>
