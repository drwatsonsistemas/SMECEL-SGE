...
$colname_professores = "-1";
if (isset($_GET['sec'])) {
  $colname_professores = $_GET['sec'];
}
mysql_select_db($database_SmecelNovo, $SmecelNovo);
$query_professores = sprintf("
SELECT vinculo_id, vinculo_id_escola, vinculo_id_sec, vinculo_id_funcionario, vinculo_id_funcao, vinculo_carga_horaria, vinculo_data_inicio, vinculo_obs, func_id, func_nome, func_email, func_senha, func_senha_ativa, funcao_id, funcao_docencia 
FROM smc_vinculo
INNER JOIN smc_func ON func_id = vinculo_id_funcionario
INNER JOIN smc_funcao ON funcao_id = vinculo_id_funcao 
WHERE func_senha_ativa = '1' AND vinculo_id_sec = %s
...
<main class="ls-main">
  <div class="container-fluid">
    <h1 class="ls-title-intro ls-ico-home">PROFESSORES / ACESSO</h1>
    <div class="ls-box ls-board-box"> 
    <!-- CONTEUDO -->
    
    <?php if ($totalRows_professores==0) { ?>

    <p class="ls-txt-center">Nenhum professor com senha ativa neste município</p>
    
    <?php } else { ?>
    
    <p class="ls-txt-center">Foram encontrados <?php echo $totalRows_professores; ?> professores com senha ativa nesta entidade</p>
    ...
    <table class="ls-table ls-table-striped fonte-tabela">
      ...
      <?php do { ?>
        <tr>
          ...
          <td><?php echo $row_professores['func_senha']; ?></td>
          <td><a href="https://www.smecel.com.br/professor/index.php?codigo=<?php echo $row_professores['func_id']; ?>&email=<?php echo $row_professores['func_email']; ?>&senha=<?php echo $row_professores['func_senha']; ?>" target="_blank">LINK</a></td>
        </tr>
