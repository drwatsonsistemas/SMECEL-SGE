...
if (isset($_GET['cidade'])) {
  ...
  $cidade = isset($_GET['cidade']) ? intval($_GET['cidade']) : -1;
...
SELECT 
    ...
    COUNT(*) AS total_accesses, login_aluno_ano,
    vinculo_aluno_id_aluno, vinculo_aluno_id_sec
FROM smc_login_aluno
INNER JOIN smc_vinculo_aluno ON vinculo_aluno_id_aluno = login_aluno_id_aluno
WHERE DATE(login_aluno_data_hora) = '$dataEscolhida' AND vinculo_aluno_id_sec = '$cidade'
...
SELECT 
    ...
    vinculo_id_funcionario, vinculo_id_sec
FROM smc_login_professor 
INNER JOIN smc_vinculo ON vinculo_id_funcionario = login_professor_id_professor
WHERE DATE(login_professor_data_hora) = '$dataEscolhida' AND vinculo_id_sec = '$cidade'
...
WHERE DATE(log_data_hora) = '$dataEscolhida' AND escola_id_sec = '$cidade'
...
<main class="ls-main">
  ...
  <form action="login_hora_cidade.php?cidade=<?php echo $cidade; ?>" class="ls-form ls-form-inline">
    <label class="ls-label col-md-3 col-sm-4">
      <b class="ls-label-text">Data</b>
      <input type="date" name="data" value="<?php echo $dataEscolhida; ?>" required pattern="\d{4}-\d{2}-\d{2}">
    </label>
    ...
    <div class="ls-actions-btn">
      ...
      <a href="login_hora_cidade.php?cidade=<?php echo $cidade; ?>" class="ls-btn">Hoje</a>
    </div>
  </form>
</div>
