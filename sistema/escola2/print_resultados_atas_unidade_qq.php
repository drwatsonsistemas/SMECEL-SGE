...
<body onload="self.print();">
		...
		<div class="ls-box1 ls-txt-center">
			<table>
				<tr>
					...
					<p>
						<h2 class="ls-txt-center">ATA DE RESULTADOS DO <?php echo $codUnidade; ?>ª TRIMESTRE</h2>
					</p>
					<p>
						<div style="text-align:justify; line-height:150%;"> Apuração das notas finais do <strong><?php echo $codUnidade; ?>ª Trimestre</strong> do Ano Letivo de <strong><?php echo $row_AnoLetivoDesc['ano_letivo_ano']; ?></strong> dos alunos da turma <strong><?php echo $row_AlunoBoletim['turma_nome']; ?></strong>, turno <strong><?php echo $row_AlunoBoletim['turma_turno_nome']; ?></strong>, deste estabelecimento de ensino, com os seguintes resultados: <br>
						</div>
					</p>
					<?php 
    ...
    // Verifica aprovação com paralela ou recuperação
							            if ($totalTrimestre >= $mediaTrimestre) {
							            	$classeNota = 'nota-apr';
							            	$alunos_aprovados[] = array('aluno_id' => $row_AlunoBoletim['vinculo_aluno_id'], 'periodo' => $unidade);
							            } else if ($notaParalela > 0 && $notaParalela >= 8.1 && $totalTrimestreSemRecuperacao >= $mediaTrimestre) {
							            	$classeNota = 'nota-paralela-apr';
							            	$alunos_aprovados[] = array('aluno_id' => $row_AlunoBoletim['vinculo_aluno_id'], 'periodo' => $unidade);
							            } else {
							            	$alunos_reprovados_paralela[] = array('aluno_id' => $row_AlunoBoletim['vinculo_aluno_id'], 'periodo' => $unidade);
							            }
							            ...
							            ?>
							            <td style="color:<?= $cor ?>"><?= $totalTrimestre ?></td>
