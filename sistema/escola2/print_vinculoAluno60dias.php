...
$dataAntes = date('Y-m-d', strtotime('-60 days'));
...
<script>
  ...
  gtag('config', 'UA-117872281-1');
    ...
    <title>RELATÓRIO DE ALUNOS MATRICULADOS HÁ 60 DIAS (<?php echo $nomeFiltro; ?>) | SMECEL - Sistema de Gestão Escolar</title>
  ...
  <body onload="self.print();">
      ...
      <div class="container-fluid">
		...
		<?php echo $row_EscolaLogada['escola_telefone1']; ?> <?php echo $row_EscolaLogada['escola_telefone2']; ?> <?php echo $row_EscolaLogada['escola_email']; ?>
			...
			<h4>RELATÓRIO DE ALUNOS MATRICULADOS HÁ 60 DIAS (<?php echo $nomeFiltro; ?>)</h4><br>
			
            <h5>Entre <?php echo date("d/m/Y", strtotime($dataAntes)) ?> e <?php echo date("d/m/Y", strtotime($dataAgora)) ?></h5>
		...
		SELECT 
		vinculo_aluno_id, vinculo_aluno_id_aluno, vinculo_aluno_id_turma, vinculo_aluno_id_escola, vinculo_aluno_ano_letivo, vinculo_aluno_data,
		...
		aluno_id, aluno_nome,  aluno_nome_social,aluno_nascimento, aluno_filiacao1, aluno_hash
		FROM smc_vinculo_aluno 
		INNER JOIN smc_aluno ON aluno_id = vinculo_aluno_id_aluno
		LEFT JOIN smc_etapa ON etapa_id = vinculo_aluno_multietapa  
		WHERE vinculo_aluno_data BETWEEN '$dataAntes' AND '$dataAgora' AND vinculo_aluno_ano_letivo = '$row_AnoLetivo[ano_letivo_ano]' AND vinculo_aluno_id_turma = '$row_ExibirTurmas[turma_id]' $stqry
		...
		<thead>
			<tr>
				...
				<th class="ls-txt-center hidden-xs" width="130">MATRÍCULA</th>
			...
			<?php do { ?>
				<tr style="border-bottom:black solid 1 px;">
					...
					$contaAlunos++;		
					?></td>
					<td><?php echo $row_ExibirAlunosVinculados['aluno_nome_social'] != "" ? $row_ExibirAlunosVinculados["aluno_nome"] : $row_ExibirAlunosVinculados["aluno_nome"]; ?> 
          			<?php if (($row_ExibirTurmas['turma_multisseriada']==1) && ($row_ExibirAlunosVinculados['vinculo_aluno_multietapa']==0)) { ?>
                     | <i class="ls-color-danger">*informe a etapa do aluno na turma multi</i> 
					<?php } else { ?>
                     | <b class="ls-color-success"><?php echo $row_ExibirAlunosVinculados['etapa_nome_abrev']; ?></b> 
					<?php } ?></td>
					...
					<td class="ls-txt-center hidden-xs"><?php echo $row_ExibirAlunosVinculados['vinculo_aluno_data']; ?></td>
