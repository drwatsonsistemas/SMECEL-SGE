...
header("Location:boletimResetarBoletimConceito.php?turma=$colname_Turma");  
die;
...
echo "
<br>
<p style='text-align:center; float:center;'>No momento, só é permitido resetar boletins para turma de Ensino Fundamental que são avaliados por notas.</p>
...
<?php 
          ...
          $deleteSQL = sprintf("DELETE FROM smc_nota WHERE nota_id_matricula = %s",
            GetSQLValueString($idVinculo, "int"));
          mysql_select_db($database_SmecelNovo, $SmecelNovo);
          $Result2 = mysql_query($deleteSQL, $SmecelNovo) or die(mysql_error());

  			  $updateSQL = sprintf("UPDATE smc_vinculo_aluno SET vinculo_aluno_boletim='0' WHERE vinculo_aluno_id=%s",
                       GetSQLValueString($idVinculo, "int"));
