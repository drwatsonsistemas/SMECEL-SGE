...
if (isset($_GET['hash'])) {
  ...
  $codMatricula = anti_injection($_GET['hash']);
  //$codMatricula = (int)$codMatricula;
  //echo $buscaTurma = "AND turma_id = $codTurma AND vinculo_aluno_hash = $codMatricula";  
  $buscaTurma = "AND turma_id = $codTurma AND vinculo_aluno_hash = '$_GET[hash]'";
}
...
escola_telefone2, escola_email, escola_inep, escola_cnpj, escola_logo, escola_tema,  sec_id, sec_cidade, sec_uf, sec_nome 
FROM smc_escola
...
<title><?php echo "FREQUENCIA ESCOLAR INDIVIDUAL $anoLetivo - $row_AlunoBoletim[aluno_nome] - $row_AlunoBoletim[turma_nome] - $row_EscolaLogada[escola_nome]" ?></title>
<meta charset="utf-8">
				...
				?>
				  ...
				  ?>
                    ...
                    <span class="ls-float-right" style="margin-left:20px;">
                      ...
                      <h4 class="ls-txt-center"><?php echo mb_strtoupper($row_EscolaLogada['sec_nome']); ?></h4>
                      <h5 class="ls-txt-center"><?php echo $row_EscolaLogada['escola_nome']; ?></h5><br>
                      <small> Aluno(a): <strong><?php if($row_AlunoBoletim['aluno_nome_social'] == ''){ echo $row_AlunoBoletim['aluno_nome']; }else{ echo $row_AlunoBoletim['aluno_nome_social']; } ?></strong> |
                      Nascimento: <strong><?php echo inverteData($row_AlunoBoletim['aluno_nascimento']); ?></strong> | 
                      Filiação: <strong><?php echo $row_AlunoBoletim['aluno_filiacao1']; ?></strong><br>  
                      Turma: <strong><?php echo $row_AlunoBoletim['turma_nome']; ?></strong> | Data da matrícula: <strong><?php echo date("d/m/Y", strtotime($row_AlunoBoletim['vinculo_aluno_data'])); ?></strong>
                      
					 
					  <?php if ($row_AlunoBoletim['vinculo_aluno_situacao']<>"1") { ?><br><strong>Matrícula encerrada em <?php echo date("d/m/Y", strtotime($row_AlunoBoletim['vinculo_aluno_datatransferencia'])); ?></strong><?php } ?>
                      </small>
					...
					<?php $dataAgora = $anoInicio."-".str_pad($mesInicio, 2, "0", STR_PAD_LEFT)."-".str_pad($diaCont, 2, "0", STR_PAD_LEFT); ?>
						  ...
						  <?php 
								...
								$dataHoje = date("Y-m-d");
									...
									//$dataHoje = date("Y-m-d", strtotime($row_AlunoBoletim['vinculo_aluno_datatransferencia']));
									$dataHoje = date("Y-m-d",strtotime(date($row_AlunoBoletim['vinculo_aluno_datatransferencia'])."-1 day"));

								//date("d/m/Y",strtotime(date("Y-m-d")."+12 month"))

								}
								
								if ($dataAgora > $dataHoje) {
