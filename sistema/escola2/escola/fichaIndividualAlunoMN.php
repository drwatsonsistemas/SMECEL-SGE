...
vinculo_aluno_transporte, vinculo_aluno_data, vinculo_aluno_hash, vinculo_aluno_verificacao, vinculo_aluno_boletim, vinculo_aluno_conselho, 
vinculo_aluno_situacao, vinculo_aluno_datatransferencia, vinculo_aluno_da_casa, vinculo_aluno_historico_transferencia, vinculo_aluno_conselho_parecer,
...
COUNT(IF(month(faltas_alunos_data) = 1, faltas_alunos_data, NULL )) janeiro,
COUNT(IF(month(faltas_alunos_data) = 2, faltas_alunos_data, NULL )) fevereiro,
COUNT(IF(month(faltas_alunos_data) = 3, faltas_alunos_data, NULL )) marco,
COUNT(IF(month(faltas_alunos_data) = 4, faltas_alunos_data, NULL )) abril,
COUNT(IF(month(faltas_alunos_data) = 5, faltas_alunos_data, NULL )) maio,
COUNT(IF(month(faltas_alunos_data) = 6, faltas_alunos_data, NULL )) junho,
COUNT(IF(month(faltas_alunos_data) = 7, faltas_alunos_data, NULL )) julho,
COUNT(IF(month(faltas_alunos_data) = 8, faltas_alunos_data, NULL )) agosto,
COUNT(IF(month(faltas_alunos_data) = 9, faltas_alunos_data, NULL )) setembro,
COUNT(IF(month(faltas_alunos_data) = 10, faltas_alunos_data, NULL )) outubro,
COUNT(IF(month(faltas_alunos_data) = 11, faltas_alunos_data, NULL )) novembro,
COUNT(IF(month(faltas_alunos_data) = 12, faltas_alunos_data, NULL )) dezembro,
...
<head>
    ...
    <title>
        <?php echo "FICHA INDIVIDUAL DO ALUNO - $row_matricula[aluno_nome] - $row_matricula[turma_nome] - $row_EscolaLogada[escola_nome]" ?>
    </title>
    <meta charset="utf-8">
...
<body onLoad="print()">
    ...
    <table width="100%">
        ...
        <tr>
            <td colspan="3"><br>
                <h2 class="ls-txt-center">FICHA INDIVIDUAL DO ALUNO</h2>
            </td>
        </tr>
            ...
            do { ?>
                <tr>
                    ...
                    for ($p = 1; $p <= $row_criteriosAvaliativos['ca_qtd_periodos']; $p++) {
                        ...
                        // Lógica de aprovação, reprovação e paralela
                        $classeNota = '';

                        if ($notaRecuperacao > 0) {
                            switch ($p) {
                                case '1':
                                    $pontuacaoTotalAnoLetivo1 = $notaRecuperacao;
                                    break;
                                case '2':
                                    $pontuacaoTotalAnoLetivo2 = $notaRecuperacao;
                                    break;
                                case '3':
                                    $pontuacaoTotalAnoLetivo3 = $notaRecuperacao;
                                    break;
                            }
                            if ($notaRecuperacao < $mediaTrimestre) {
                            ...
                            if (($somaPontuacaoQuantitativo < 8.1 && $totalTrimestreSemRecuperacao < $mediaTrimestre)) {
                                $alunos_reprovados_paralela[] = array('aluno_id' => $row_matricula['vinculo_aluno_id'], 'periodo' => $p);
                                $classeNota = 'nota-paralela-rep';
                            } elseif (($somaPontuacaoQuantitativo >= 8.1 && $totalTrimestreSemRecuperacao >= $mediaTrimestre)) {
                                $alunos_aprovados[] = array('aluno_id' => $row_matricula['vinculo_aluno_id'], 'periodo' => $p);
                                $classeNota = 'nota-apr';
                            } elseif ($notaParalela > 0 && ($notaParalela >= 8.1 && $totalTrimestreSemRecuperacao >= $mediaTrimestre)) {
                                $alunos_aprovados[] = array('aluno_id' => $row_matricula['vinculo_aluno_id'], 'periodo' => $p);
                                $classeNota = 'nota-paralela-apr';
                            }
                        }

                        // Verificar se a nota total do trimestre é maior ou igual à média necessária
                        if ($totalTrimestre >= $mediaTrimestre) {
                            $classeNota = 'nota-apr'; // Classe verde indicando aprovação
                        }
                    ...
                    <td width="15" inputmode="numeric" disciplina="<?php echo $row_Disciplina['disciplina_nome']; ?>"
                        class="ls-txt-center"><span class="ls-text-md">
    ...
    <small>
        TT: Total trimestre | TP: Total de pontos | REC: Recuperação do trimestre | RF: Resultado final
    </small>
    <br>
    <p class="ls-txt-center">FREQUÊNCIA</p>
    <table class="bordasimples" width="100%">
        ...
        <tr>
            ...
            <td class="ls-txt-center"><?php echo ($row_Faltas['fevereiro'] > 0) ? $row_Faltas['fevereiro'] : '-'; ?>
            </td>
        ...
        <tr>
            <td colspan="12" class="ls-txt-center">TOTAL DE FALTAS: <strong><?php echo $row_Faltas['total']; ?></strong>
            </td>
        </tr>
    ...
    <table class="ls-table1 ls-sm-space bordasimples" width="100%">
        ...
        <?php if ($row_matricula['vinculo_aluno_conselho_parecer'] != '') { ?>
            <tr>
                <td colspan="2">Parecer do conselho de classe:
                    <br><br><?php echo $row_matricula['vinculo_aluno_conselho_parecer']; ?><br><br></td>
            </tr>
        <?php } ?>
        <tr>
    ...
    <p style="text-align:center">
        ...
        <br><br> SMECEL - Sistema de Gestão Escolar | www.smecel.com.br | <small>Código de certificação:
            <strong><?php echo $row_matricula['vinculo_aluno_verificacao']; ?></strong></small>
    </p>
