...
mysql_select_db($database_SmecelNovo, $SmecelNovo);
$query_Turmas = "SELECT turma_id, turma_id_escola, turma_nome, turma_etapa, turma_turno, turma_total_alunos, turma_ano_letivo FROM smc_turma WHERE turma_ano_letivo = '$row_AnoLetivo[ano_letivo_ano]' AND turma_id_escola = '$row_EscolaLogada[escola_id]'
...
<main class="ls-main ">
  ...
  <h1 class="ls-title-intro ls-ico-home">Vincular</h1>
  <!-- CONTEÚDO -->
  ...
  <form method="post" name="form1" action="<?php echo $editFormAction; ?>" class="ls-form ls-form-horizontal row" data-ls-module="form" autocomplete="off">
    <fieldset>
      <div class="ls-box">
        ...
        <label class="ls-label col-md-3"><b class="ls-label-text">DATA DA MATRÍCULA</b>
          <input type="date" placeholder="INFORME A DATA" name="vinculo_aluno_data" id="data_matricula1" value="<?php echo date("Y-m-d"); ?>" class="ls-field-lg" required>
        </label>
        ...
        <select name="vinculo_aluno_id_turma" class="ls-select" required>
          ...
          <?php if ($row_Turmas['turma_turno']=="1") { echo "MAT"; } else if ($row_Turmas['turma_turno']=="2") { echo "VESP"; } else { echo "NOT"; }; ?>
          /
          <?php 
      ...
      <b class="ls-label-text">TRANSPORTE ESCOLAR</b><a href="#" class="ls-ico-help" data-trigger="hover" data-ls-module="popover" data-placement="left" data-content="Informar apenas o aluno que mora na Zona Rural." data-title="Atenção"></a> <br>
      <br>
      <p>
        <label class="ls-label-text">
          <input type="radio" name="vinculo_aluno_transporte" value="N" onclick="javascript:transporte_nao();" checked />
          NÃO UTILIZA</label>
        <label class="ls-label-text">
          <input type="radio" name="vinculo_aluno_transporte" value="S" onclick="javascript:transporte_sim();" />
          UTILIZA </label>
      </p>
      </label>
      </div>
      ...
      <div class="ls-custom-select ls-field-lg">
        <select name="vinculo_aluno_ponto_id" class="ls-select">
          <option value="" <?php if (!(strcmp(-1, ""))) {echo "SELECTED";} ?>>-</option>
          <?php do { ?>
          <option value="<?php echo $row_Pontos['te_ponto_id']?>"><?php echo $row_Pontos['te_ponto_descricao']?> <?php echo $row_Pontos['te_ponto_endereco']?> <?php echo $row_Pontos['te_ponto_num']?> <?php echo $row_Pontos['te_ponto_bairro']?> <?php echo $row_Pontos['te_ponto_obs']?></option>
      ...
      <b class="ls-label-text">Aluno repetente nessa etapa de ensino? </b> <a href="#" class="ls-ico-help" data-trigger="hover" data-ls-module="popover" data-placement="right" data-content="Informar se o aluno repetiu esse mesmo ano/série." data-title="Atenção"></a> <br>       <br>
      <p>
        <label class="ls-label-text">
      ...
      <b class="ls-label-text">TRANSFERIDO COM HISTÓRICO OU DECLARAÇÃO</b><a href="#" class="ls-ico-help" data-trigger="hover" data-ls-module="popover" data-placement="left" data-content="Se o aluno foi trasferido com Declaração, terá 30 dias a contar da data da matrícula para entregar o Histórico" data-title="Atenção"></a> <br>
      <br>
      <p>
        <label class="ls-label-text">
      ...
      <p>
        <label class="ls-label-text">
          <input type="radio" name="vinculo_aluno_vacina_atualizada" value="S" onclick="javascript:aceite();" required />
          SIM </label>
        <label class="ls-label-text">
      ...
      <p>
      <div class="ls-alert-warning ls-dismissable" style="display:none" id="aviso_aceite"> <span data-ls-module="dismiss" class="ls-dismiss">&times;</span> <strong>Atenção!</strong> Ao marcar SIM, você DECLARA que o aluno ou responsável pelo aluno apresentou o documento que comprove que a Carteira de Vacinação está em dia. </div>
      </p>
      <div style="display:none" id="aviso_aceite_1">
        <label class="ls-label col-md-5"> <b class="ls-label-text">DATA DO RETORNO PARA PRÓXIMA VACINA</b>
          <input type="date" placeholder="INFORME A DATA" name="vinculo_aluno_vacina_data_retorno" id="data_retorno1" value="" class="ls-field-lg">
        </label>
      </div>
      </label>
      ...
      <label class="ls-label col-sm-12 ls-box">
      <b class="ls-label-text">TURMA MULTISSERIADA</b> <a href="#" class="ls-ico-help" data-trigger="hover" data-ls-module="popover" data-placement="left" data-content="Informar se o aluno para a matrícula é de casa (já estuda na escola) ou de fora (vindo de outra escola no município ou fora dele." data-title="Atenção"></a> <br>
      <br>
      <p>
        <label class="ls-label-text">
          <input type="radio" name="vinculo_aluno_multietapa" value="S" onclick="javascript:multi_sim();" checked />
          SIM </label>
        <label class="ls-label-text">
    ...
    <div class="ls-actions-btn">
      ...
      <input value="SALVANDO..." class="ls-btn-primary ls-btn-lg ls-disabled ls-float-left" id="botaoMatricularAguarde" style="display:none; float:left">
      <a href="vinculoAlunoExibirTurma.php" class="ls-btn-danger ls-btn-lg ls-txt-right ls-float-right">Cancelar</a> </div>
    ...
    <input type="hidden" name="vinculo_aluno_ano_letivo" value="<?php echo $row_AnoLetivo['ano_letivo_ano']; ?>">
  ...
  <div class="ls-alert-alert"> <strong>Atenção:</strong> Foram encontrados os seguintes vínculos para <?php echo $row_VinculosAnteriores['aluno_nome']; ?> neste ano letivo de <?php echo $row_VinculosAnteriores['vinculo_aluno_ano_letivo']; ?>:
    <table width="100%" class="ls-table ls-table-striped ls-sm-space">
      <thead>
        <tr>
          ...
          <th class="ls-txt-center">SITUAÇÃO
            </td>
        </tr>
      </thead>
      <tbody>
