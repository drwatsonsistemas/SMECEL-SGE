...
$startDate = isset($_GET['inicio']) ? anti_injection($_GET['inicio']) : null;
$endDate = isset($_GET['fim']) ? anti_injection($_GET['fim']) : null;

$periodo_texto = "";
if($startDate && $endDate) {
	$datainicial = inverteData($startDate);
	$datafinal = inverteData($endDate);
	$periodo_texto = "Período: $datainicial - $datafinal";
}

// Adicionar filtro de data à query, se aplicável
$dateFilter = "";
if ($startDate && $endDate) {
	$dateFilter = " AND faltas_alunos_data BETWEEN '$startDate' AND '$endDate'";
}
...
<head>
	...
	<title>Alunos faltosos | SMECEL - Sistema de Gestão Escolar</title>
...
<body onload="self.print();">
	...
	<div class="container-fluid">
		...
		<div class="ls-box ls-txt-center" style="text-transform: uppercase;">
			RELATÓRIO DE TOTAL DE FALTAS POR ALUNO NO ANO LETIVO <?= $anoLetivo ?><br>
			<small><?= $periodo_texto ?></small>
		</div>
  ...
  SELECT 
    ...
    aluno_nome_social,
    COUNT(faltas_alunos_id) as total_faltas, faltas_alunos_justificada
  FROM smc_vinculo_aluno 
  INNER JOIN smc_aluno ON aluno_id = vinculo_aluno_id_aluno 
  LEFT JOIN smc_faltas_alunos ON faltas_alunos_matricula_id = vinculo_aluno_id
			...
			<?php if ($totalRows_ExibirAlunosVinculados > 0) { ?>
				...
				<div class="ls-box ls-sm-space">
					...
					<h5 class="ls-title-5 ls-txt-center"><?php echo $row_ExibirTurmas['turma_nome']; ?> -
						<?php echo $row_ExibirTurmas['turma_turno_nome']; ?> </h5>
					...
					<table class="ls-sm-space bordasimples" width="100%" style="font-size:12px;">
						<thead>
							<tr>
								...
								<th width="15%" class="ls-txt-center">DATA NASCIMENTO</th>
								<th width="15%" class="ls-txt-center">TOTAL DE FALTAS</th>
						...
						<tbody>
							<?php do { ?>
								<tr style="border-bottom:black solid 1 px;">
									...
									<td class="ls-txt-center"><?php echo $row_ExibirAlunosVinculados['total_faltas']; ?></td>
					...
					<p>Alunos com faltas na turma: <strong><?php echo $totalRows_ExibirAlunosVinculados; ?></strong></p>
