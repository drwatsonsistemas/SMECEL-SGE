...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE smc_plano_aula SET plano_aula_atividade=%s WHERE plano_aula_id=%s",
...
mysql_select_db($database_SmecelNovo, $SmecelNovo);
$query_Plano = sprintf("SELECT plano_aula_id, plano_aula_id_turma, plano_aula_id_disciplina, plano_aula_id_professor, plano_aula_data, plano_aula_texto, plano_aula_conteudo, plano_aula_atividade, plano_aula_hash FROM smc_plano_aula WHERE plano_aula_hash = %s", GetSQLValueString($colname_Plano, "text"));
...
<h6><?php echo $row_Escolas['escola_nome']; ?><br><small><?php echo $row_Disciplina['disciplina_nome']; ?> - <?php echo $row_Turma['turma_nome']; ?></small></h6>
	  ...
	  <h5>AULA: <?php echo $row_Plano['plano_aula_texto']; ?></h5>


	  
	  
     <form method="post" name="form1" action="<?php echo $editFormAction; ?>" class="row">
	   ...
	   <textarea name="plano_aula_atividade" rows="10" id="mytextarea" class="materialize-textarea">
	   <?php echo htmlentities($row_Plano['plano_aula_atividade'], ENT_COMPAT, ''); ?>
	   </textarea>
       ...
       <input type="submit" value="SALVAR ATIVIDADE" class="waves-effect waves-light btn">

	   <a href="plano_aula.php?turma=<?php echo $row_Plano['plano_aula_id_turma']; ?>&disciplina=<?php echo $row_Plano['plano_aula_id_disciplina']; ?>" class="waves-effect waves-light right btn orange">VOLTAR</a>	
...
$('.modal').modal();
	...
	tinymce.init({
	  selector: '#mytextarea',
	  height: 500,
	  toolbar: ['undo redo | formatselect | bold italic backcolor | bullist numlist | image | alignleft aligncenter alignright alignjustify | link h2 h3 blockquote'],
	  plugins : ['advlist autolink link image imagetools lists charmap print preview paste',
