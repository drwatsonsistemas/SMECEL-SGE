...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE smc_plano_aula SET plano_aula_data=%s, plano_aula_texto=%s, plano_aula_conteudo=%s, plano_aula_atividade=%s, plano_aula_atividade_resposta_obrigatoria=%s, plano_aula_atividade_resposta_obrigatoria_data_limite=%s, plano_aula_video=%s, plano_aula_meet=%s, plano_aula_sicrona_hora=%s, plano_aula_sicrona_minuto=%s, plano_aula_google_form=%s, plano_aula_google_form_tempo=%s, plano_aula_publicado=%s WHERE plano_aula_id=%s",
                       ...
                       GetSQLValueString(isset($_POST['plano_aula_meet']) ? "true" : "", "defined","'S'","'N'"),
                       GetSQLValueString($_POST['plano_aula_sicrona_hora'], "text"),
                       GetSQLValueString($_POST['plano_aula_sicrona_minuto'], "text"),
...
plano_aula_video, plano_aula_meet, plano_aula_sicrona_hora, plano_aula_sicrona_minuto, plano_aula_google_form, plano_aula_google_form_tempo, plano_aula_publicado, plano_aula_hash 
FROM smc_plano_aula 
...
mysql_select_db($database_SmecelNovo, $SmecelNovo);
$query_VideoAula = "SELECT * FROM smc_videoaula WHERE videoaula_id_aula = '$row_Plano[plano_aula_id]'";
...
if ((isset($_GET['video'])) && ($_GET['video'] != "")) {
  $deleteSQL = sprintf("DELETE FROM smc_videoaula WHERE videoaula_id_aula = '$row_Plano[plano_aula_id]' AND videoaula_id=%s",
                       GetSQLValueString($_GET['video'], "int"));

  mysql_select_db($database_SmecelNovo, $SmecelNovo);
  $Result1 = mysql_query($deleteSQL, $SmecelNovo) or die(mysql_error());

  $deleteGoTo = "plano_aula_editar.php?hash=$colname_Plano&videoDeletado";
  if (isset($_SERVER['QUERY_STRING'])) {
    //$deleteGoTo .= (strpos($deleteGoTo, '?')) ? "&" : "?";
    //$deleteGoTo .= $_SERVER['QUERY_STRING'];
  }
  ...
  <ul class="collapsible expandable">
    <li>
      ...
      <div class="collapsible-body"><span>
	   ...
	   <textarea name="plano_aula_conteudo" rows="10" id="summernote" class="materialize-textarea"><?php echo htmlentities($row_Plano['plano_aula_conteudo'], ENT_COMPAT, ''); ?></textarea>
	   </div>
	   </div>
    ...
    <li>
      <div class="collapsible-header"><i class="material-icons black-text darken-4">smartphone</i>VÍDEO DO CELULAR <?php if ($totalRows_VideoAula > 0) { ?><span class="badge" data-badge-caption=""><i class="material-icons green-text darken-4">check_circle</i></span><?php } ?></div>
      <div class="collapsible-body"><span>
	  
	  <p>
	  
	  <a class="waves-effect waves-light btn modal-trigger" href="#modal_videoaula">ENVIAR NOVO VÍDEO GRAVADO</a>
...
<video width="100%" controls>
  <source src="../../videoaula/<?php echo $row_Turma['turma_id']; ?>/<?php echo $row_Plano['plano_aula_id']; ?>/<?php echo $row_ProfLogado['func_id']; ?>/<?php echo $row_Disciplina['disciplina_id']; ?>/<?php echo $row_VideoAula['videoaula_nome']; ?>" type="video/mp4">
Seu navegador não suporta estes arquivos.
</video>

<a class="red-text btn-flat" href="plano_aula_editar.php?hash=<?php echo $colname_Plano; ?>&video=<?php echo $row_VideoAula['videoaula_id']; ?>"><i class="tiny material-icons left">delete_forever</i> Deletar vídeo</a>
</div>
...
<?php } ?> 
    ...
    <li>
      <div class="collapsible-header"><i class="material-icons red-text darken-4">ondemand_video</i>VÍDEO DO YOUTUBE <?php if ($row_Plano['plano_aula_video']<>"") { ?><span class="badge" data-badge-caption=""><i class="material-icons green-text darken-4">check_circle</i></span><?php } ?></div>
      <div class="collapsible-body"><span>
    ...
    <li>
      <div class="collapsible-header"><i class="material-icons red-text darken-4">videocam</i>AULA AO VIVO <?php if ($row_Plano['plano_aula_meet']<>"N") { ?><span class="badge" data-badge-caption=""><i class="material-icons green-text darken-4">check_circle</i></span><?php } ?></div>
      <div class="collapsible-body"><span>
  ...
  <p>Marque a opção abaixo se for transmitir uma aula síncrona (ao vivo):</p>  
  
	<div class="switch">
	<label>
      Não
      <input type="checkbox" name="plano_aula_meet" value=""  <?php if (!(strcmp(htmlentities($row_Plano['plano_aula_meet'], ENT_COMPAT, 'utf-8'),"S"))) {echo "checked=\"checked\"";} ?>>
  ...
  <br>
		  ...
		  <p>Defina a hora que o link para a aula estará disponível:</p>  
		  
		  <div class="input-field col s2 m2">
			<select name="plano_aula_sicrona_hora" id="plano_aula_sicrona_hora">
			  <option value="<?php echo $row_Plano['plano_aula_sicrona_hora']; ?>" select><?php echo $row_Plano['plano_aula_sicrona_hora']; ?></option>
		  ...
		  <div class="input-field col s2 m2">
			<select name="plano_aula_sicrona_minuto" id="plano_aula_sicrona_minuto">
			  <option value="<?php echo $row_Plano['plano_aula_sicrona_minuto']; ?>" select><?php echo $row_Plano['plano_aula_sicrona_minuto']; ?></option>
			  <option value="00">00</option>
			  <option value="05">05</option>
			  <option value="10">10</option>
			  <option value="15">15</option>
	...
	<div class="row">
		<div class="input-field col s12">
			<p>Entre na aula minutos antes, e configure as opções de segurança:</p>
			<a href="https://meet.jit.si/<?php echo $row_Plano['plano_aula_id']; ?>-<?php echo url_limpa($row_Escolas['escola_nome']); ?>-<?php echo url_limpa($row_Disciplina['disciplina_nome']); ?>-<?php echo url_limpa($row_Turma['turma_nome']); ?>" target="_blank">https://alpha.jitsi.net/<?php echo $row_Plano['plano_aula_id']; ?>-<?php echo url_limpa($row_Escolas['escola_nome']); ?>-<?php echo url_limpa($row_Disciplina['disciplina_nome']); ?>-<?php echo url_limpa($row_Turma['turma_nome']); ?></a></p>  
		</div>
	</div>
	...
	<blockquote class="card-panel deep-purple lighten-5">
	DICAS:<br>
	<ul>
	<li>-Se você marcar a opção de aula síncrona, o link acima aparecerá automaticamente no painel do aluno.</li>
	<li>-Se precisar, deixe instruções no campo "Conteúdo para o aluno", sobre o horário da aula ao vivo, regras etc. (Lembre-se que o link já estará disponível para o aluno na data da aula).</li> 	
	<li>-Faça um teste antes, clicando no link acima e configurando seu equipamento (áudio e vídeo).</li> 	
	<li>-Você pode seguir as instruções ao clicar no link, baixando o APP ou acessando diretamente pelo navegador.</li> 	
    ...
    <li>
	  ...
	  Fica a seu critério a forma como o aluno vai responder: copiando a atividade no caderno ou responder diretamente no livro, depois, enviar uma foto através do campo de envio de atividades.</p>
	   ...
	   <div class="row">
	   <div class="input-field col s12">
	   <textarea name="plano_aula_atividade" rows="10" id="summernote1" class="materialize-textarea"><?php echo htmlentities($row_Plano['plano_aula_atividade'], ENT_COMPAT, ''); ?></textarea>
	   </div>
	   </div>
...
<?php } ?>      
     ...
     <div class="input-field col s6 m8">
          <b>Tempo para o aluno responder a prova (em minutos)</b><br>
          <i>Deixe em branco para tempo livre</i>
</div>

     <div class="input-field col s6 m4">
          <input type="number" name="plano_aula_google_form_tempo" value="<?php echo $row_Plano['plano_aula_google_form_tempo']; ?>" id="plano_aula_google_form_tempo" min="0" max="999" step="5">
  	   ...
  	   <div class="row">
	   <div class="input-field col s12">
	   <h5>Liberar aula? <br><small>(só será exibida na data especificada)</small><i class="material-icons tooltipped" data-position="bottom" data-tooltip="Marcando em 'SIM' o conteúdo será publicado imediatamente. Caso queira apenas salvar a aula, mas não exibir o conteúdo para os alunos neste momento, marque a opção 'NÃO'.">help</i></h5>
...
<input type="hidden" name="plano_aula_anexo_id_atividade" value="<?php echo $row_Plano['plano_aula_id']; ?>">
  ...
  <!-- Modal Structure -->
  <div id="modal_videoaula" class="modal">
    <div class="modal-content">
      <h4>Enviar vídeo aula</h4>
      <p>
...
function uploadFile(){
	...
	ajax.open("POST", "videoaula.php?aula=<?php echo $row_Plano['plano_aula_id']; ?>&professor=<?php echo $row_ProfLogado['func_id']; ?>&componente=<?php echo $row_Disciplina['disciplina_id']; ?>&turma=<?php echo $row_Turma['turma_id']; ?>");
	ajax.send(formdata);
}
...
//minDate: new Date(),
	...
	<script language="Javascript">
	function confirmaExclusaoVideo(codigo) {
     var resposta = confirm("Deseja realmente excluir este vídeo?");
     	if (resposta == true) {
     	     window.location.href = "plano_aula_editar.php?"+codigo;
    	 }
	}
...
<?php } ?>
<?php if (isset($_GET["videoDeletado"])) { ?>
  <script>
M.toast({html: '<i class=\"material-icons\">check_circle</i>&nbsp;<button class="btn-flat toast-action">VÍDEO AULA DELETADA COM SUCESSO</button>'});
</script>
<?php } ?>	
