...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE smc_func SET func_senha=%s WHERE func_id=%s",
                       GetSQLValueString($_POST['func_senha'], "text"),
...
mysql_select_db($database_SmecelNovo, $SmecelNovo);
$query_Senha = "SELECT func_id, func_senha FROM smc_func WHERE func_id = '$row_ProfLogado[func_id]'";
$Senha = mysql_query($query_Senha, $SmecelNovo) or die(mysql_error());
$row_Senha = mysql_fetch_assoc($Senha);
$totalRows_Senha = mysql_num_rows($Senha);
...
<small style="font-size:14px;">
    ...
    <form method="post" id="form1" name="form1" action="<?php echo $editFormAction; ?>" class="card-panel" onsubmit="return validarSenha();">
      <div class="row">
	  ...
	  <a href="index.php" class="waves-effect waves-light btn orange">CANCELAR</a>  
      </div>
      </div>
...
<?php include ("rodape.php"); ?>      <script type="text/javascript" src="../js/app.js"></script>
        ...
        if (nsenha != rsenha){ 
             alert("As senhas informadas não são iguais. Verifique o que digitou.");
             return false;
        }
        return true;
 }
...
<script>
M.toast({html: '<i class=\"material-icons\">check_circle</i>&nbsp;<button class="btn-flat toast-action">Senha alterada com sucesso.</button>'});
</script>
  <?php } ?>
    </body>
  </html>
