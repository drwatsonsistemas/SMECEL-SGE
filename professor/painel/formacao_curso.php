...
mysql_select_db($database_SmecelNovo, $SmecelNovo);
$query_Curso = sprintf("SELECT curso_form_id, curso_form_id_sec, curso_form_nome, curso_form_descricao, curso_form_data_inicio, curso_form_data_fim, curso_form_responsavel, curso_form_ch, curso_form_hash, curso_form_aberto FROM smc_curso_formacao WHERE curso_form_hash = %s", GetSQLValueString($colname_Curso, "text"));
$Curso = mysql_query($query_Curso, $SmecelNovo) or die(mysql_error());
$row_Curso = mysql_fetch_assoc($Curso);
$totalRows_Curso = mysql_num_rows($Curso);

mysql_select_db($database_SmecelNovo, $SmecelNovo);
$query_Topicos = "SELECT curso_form_topic_id, curso_form_topic_id_form, curso_form_topic_titulo, curso_form_topic_texto, curso_form_topic_aberto, curso_form_data FROM smc_curso_formacao_topicos WHERE curso_form_topic_aberto = 'S' AND curso_form_topic_id_form = '$row_Curso[curso_form_id]' ORDER BY curso_form_topic_id ASC";
...
<small style="font-size:14px;">
    ...
    <a href="formacoes.php" class="waves-effect waves-light btn-small btn-flat"><i class="material-icons left">arrow_back</i> Voltar</a>
	  ...
	  <h5><?php echo $row_Curso['curso_form_nome']; ?></h5>
  ...
  <table>
    <?php do { ?>
      <tr>
        <td width="100"><strong> <?php echo date("d/m/Y", strtotime($row_Topicos['curso_form_data'])); ?></strong> </td><td> <a href="formacao_topico.php?codigo=<?php echo $row_Topicos['curso_form_topic_id']; ?>"><?php echo $row_Topicos['curso_form_topic_titulo']; ?></a></td>
      </tr>
