...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE smc_coment_ativ_aluno SET com_at_aluno_comentario_professor=%s, com_at_aluno_comentario_professor_data=%s WHERE com_at_aluno_id=%s",
                       GetSQLValueString($_POST['com_at_aluno_comentario_professor'], "text"),
                       GetSQLValueString($_POST['com_at_aluno_comentario_professor_data'], "date"),
                       GetSQLValueString($_POST['com_at_aluno_id'], "int"));

  mysql_select_db($database_SmecelNovo, $SmecelNovo);
  $Result1 = mysql_query($updateSQL, $SmecelNovo) or die(mysql_error());

  $updateGoTo = "forum.php?respondido";
  if (isset($_SERVER['QUERY_STRING'])) {
    $updateGoTo .= (strpos($updateGoTo, '?')) ? "&" : "?";
    $updateGoTo .= $_SERVER['QUERY_STRING'];
    $updateGoTo .= "#comentario_".$_POST['com_at_aluno_id'];
  }
...
if (isset($_GET['comentario'])) {
  $colname_Comentario = anti_injection($_GET['comentario']);
}
mysql_select_db($database_SmecelNovo, $SmecelNovo);
$query_Comentario = sprintf("
SELECT com_at_aluno_id, com_at_aluno_id_atividade, com_at_aluno_id_matricula, 
com_at_aluno_data_hora, com_at_aluno_comentario, com_at_aluno_comentario_professor, 
com_at_aluno_comentario_professor_data, vinculo_aluno_id, vinculo_aluno_id_aluno, aluno_id, aluno_nome, aluno_foto
FROM smc_coment_ativ_aluno
INNER JOIN smc_vinculo_aluno ON vinculo_aluno_id = com_at_aluno_id_matricula
INNER JOIN smc_aluno ON aluno_id = vinculo_aluno_id_aluno 
WHERE com_at_aluno_id = %s", GetSQLValueString($colname_Comentario, "int"));
$Comentario = mysql_query($query_Comentario, $SmecelNovo) or die(mysql_error());
$row_Comentario = mysql_fetch_assoc($Comentario);
$totalRows_Comentario = mysql_num_rows($Comentario);

if($totalRows_Comentario=="") {
...
<small style="font-size:14px;">
    ...
    <div class="col s12 m10">
	  ...
	  <div class="col s4 m2">
				<?php if ($row_Comentario['aluno_foto']=="") { ?>
				<img src="<?php echo URL_BASE.'aluno/fotos/' ?>semfoto.jpg" width="100%" class="hoverable">
				<?php } else { ?>
				<img src="<?php echo URL_BASE.'aluno/fotos/' ?><?php echo $row_Comentario['aluno_foto']; ?>" width="100%" class="hoverable">
				<?php } ?>
		</div>			
	  
	  <div class="col s8 m10">
	  
	  <h6><?php echo $row_Comentario['aluno_nome']; ?></h6>
	  
	  <?php echo nl2br($row_Comentario['com_at_aluno_comentario']); ?>
	  
	  </div>
	  
	  </div>
	  
		<a href="forum.php?hash=<?php echo $hash; ?>#comentario_<?php echo $row_Comentario['com_at_aluno_id']; ?>" class="waves-effect waves-light btn-small btn-flat"><i class="material-icons left">arrow_back</i> Voltar</a>
        <form method="post" name="form1" action="<?php echo $editFormAction; ?>" class="card-panel">
          
          

		<div class="input-field col s12">
              <textarea class="materialize-textarea" id="textarea1" name="com_at_aluno_comentario_professor"><?php echo htmlentities($row_Comentario['com_at_aluno_comentario_professor'], ENT_COMPAT, ''); ?></textarea>
			  <label for="textarea1">Sua resposta ao comentário acima:</label>
        </div>
          ...
          <input type="hidden" name="com_at_aluno_comentario_professor_data" value="<?php echo date('Y-m-d H:i:s'); ?>">
          <input type="hidden" name="MM_update" value="form1">
          <input type="hidden" name="com_at_aluno_id" value="<?php echo $row_Comentario['com_at_aluno_id']; ?>">
        </form>
        <p>&nbsp;</p>
    </div>
