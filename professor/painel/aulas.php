...
<a href="index.php" class="waves-effect waves-light btn-small btn-flat"><i class="material-icons left">arrow_back</i> Voltar</a>    
  ...
  <div style="border: 1px solid #fafafa; padding:5px; margin:10px 0;">
		...
		<h5><i class="tiny material-icons orange-text">featured_play_list</i> Últimas aulas cadastradas <a href="atividades.php"><small>(ver todas)</small></a></h5>
        
        <?php if ($totalRows_UltimasAulas <> 0) { ?>
		<table class="striped">
          <?php do { ?>
            <tr>
              <td width="130" class="center"><?php echo inverteData($row_UltimasAulas['plano_aula_data']); ?></td>
              <td><a href="forum.php?hash=<?php echo $row_UltimasAulas['plano_aula_hash']; ?>&turma=<?php echo $row_UltimasAulas['plano_aula_id_turma']; ?>&disciplina=<?php echo $row_UltimasAulas['plano_aula_id_disciplina']; ?>"><?php echo $row_UltimasAulas['plano_aula_texto']; ?> - <?php echo $row_UltimasAulas['disciplina_nome']; ?> - <?php echo $row_UltimasAulas['turma_nome']; ?></a></td>
            </tr>
