...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE smc_plano_aula SET plano_aula_conteudo=%s WHERE plano_aula_id=%s",
...
mysql_select_db($database_SmecelNovo, $SmecelNovo);
$query_Plano = sprintf("SELECT plano_aula_id, plano_aula_id_turma, plano_aula_id_disciplina, plano_aula_id_professor, plano_aula_data, plano_aula_texto, plano_aula_conteudo, plano_aula_hash FROM smc_plano_aula WHERE plano_aula_hash = %s", GetSQLValueString($colname_Plano, "text"));
...
<h6><?php echo $row_Escolas['escola_nome']; ?><br><small><?php echo $row_Disciplina['disciplina_nome']; ?> - <?php echo $row_Turma['turma_nome']; ?></small></h6>
     ...
     <form method="post" name="form1" action="<?php echo $editFormAction; ?>" class="row">
	   ...
	   <textarea name="plano_aula_conteudo" rows="10" id="mytextarea" class="materialize-textarea">
	   <?php echo htmlentities($row_Plano['plano_aula_conteudo'], ENT_COMPAT, ''); ?>
	   </textarea>
       ...
       <input type="submit" value="INSERIR CONTEÚDO" class="waves-effect waves-light btn">
