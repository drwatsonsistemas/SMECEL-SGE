...
try {
	...
	if (isset($_GET['ct'])) {
		...
		$codTurma = (int) anti_injection($_GET['ct']);
	...
	$stmtEscolaLogada = $SmecelNovo->prepare(
		"SELECT escola_id, escola_id_sec, escola_nome, escola_cep, escola_endereco, escola_num, escola_bairro, 
         ...
         WHERE escola_id = :colname_Escola"
	);
	$stmtEscolaLogada->execute(['colname_Escola' => $colname_Escola]);
	...
	$stmtAlunoBoletim = $SmecelNovo->prepare(
		"SELECT vinculo_aluno_id, vinculo_aluno_id_aluno, vinculo_aluno_id_turma, vinculo_aluno_id_escola, 
                ...
                aluno_id, aluno_nome, aluno_nascimento, aluno_filiacao1, aluno_foto, aluno_hash, aluno_nome_social,
                turma_id, turma_nome, turma_matriz_id, turma_turno, turma_etapa 
         FROM smc_vinculo_aluno
         INNER JOIN smc_aluno ON aluno_id = vinculo_aluno_id_aluno 
         INNER JOIN smc_turma ON turma_id = vinculo_aluno_id_turma 
         WHERE vinculo_aluno_situacao = '1' AND vinculo_aluno_ano_letivo = :anoLetivo AND vinculo_aluno_id_escola = :colname_Escola $buscaTurma
         ORDER BY turma_turno ASC, turma_etapa ASC, turma_nome ASC, aluno_nome ASC"
	);

	$paramsAlunoBoletim = [
		'anoLetivo' => $anoLetivo,
		'colname_Escola' => $colname_Escola
	];
	if ($codTurma !== "") {
		$paramsAlunoBoletim['codTurma'] = $codTurma;
	}

	$stmtAlunoBoletim->execute($paramsAlunoBoletim);
	...
	$stmtMatriz = $SmecelNovo->prepare(
		"SELECT matriz_id, matriz_id_secretaria, matriz_id_etapa, matriz_nome, matriz_obs, matriz_anoletivo, matriz_hash, 
                matriz_dias_letivos, matriz_semanas_letivas, matriz_dias_semana, matriz_minutos_aula, matriz_aula_dia, matriz_criterio_avaliativo 
         FROM smc_matriz WHERE matriz_id = :turma_matriz_id"
	);
	$stmtMatriz->execute(['turma_matriz_id' => $row_AlunoBoletim['turma_matriz_id']]);
	...
	$stmtCriteriosAvaliativos = $SmecelNovo->prepare(
		"SELECT ca_id, ca_id_secretaria, ca_descricao, ca_qtd_periodos, ca_qtd_av_periodos, ca_nota_min_av, ca_nota_max_av, 
         ...
         WHERE ca_id = :criterio_avaliativo"
	);
	$stmtCriteriosAvaliativos->execute(['criterio_avaliativo' => $row_Matriz['matriz_criterio_avaliativo']]);
             ...
             GROUP BY faltas_alunos_disciplina_id"
					...
					<div class="ls-box1">
						<span class="ls-float-right" style="margin-left:20px;">
							...
							<?php } ?>
						</span> <small> Aluno(a): <strong><?php echo $row_AlunoBoletim['aluno_nome_social'] <> '' ? $row_AlunoBoletim['aluno_nome_social'] : $row_AlunoBoletim['aluno_nome']; ?></strong><br>
							Nascimento:
