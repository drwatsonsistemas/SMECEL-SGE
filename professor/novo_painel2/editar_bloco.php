...
$bloco = isset($_GET['b']) ? anti_injection($_GET['b']) : "-1";
...
try {
  // Consultar ac_label
  $stmtAcLabel = $SmecelNovo->prepare("SELECT * FROM smc_ac_label WHERE ac_id_ac = :ac_id AND ac_id_tipo = :bloco");
  $stmtAcLabel->execute([
    ':ac_id' => $colname_ac,
    ':bloco' => $bloco
  ]);
  $rowAcLabel = $stmtAcLabel->fetch(PDO::FETCH_ASSOC);
  $TotalrowAcLabel = $stmtAcLabel->rowCount();

  if ($TotalrowAcLabel === 0) {
  ...
  $stmtAc = $SmecelNovo->prepare(
    "SELECT ac_id, ac_id_professor, ac_id_escola, ac_id_componente, ac_id_etapa, ac_ano_letivo, ac_data_inicial, ac_data_final, 
        ...
        ac_tema, ac_unid_tematica FROM smc_ac WHERE ac_id = :ac_id"
  );
  $stmtAc->execute([':ac_id' => $rowAcLabel['ac_id_ac']]);
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['MM_update']) && $_POST['MM_update'] === 'form1') {
    $stmtUpdate = $SmecelNovo->prepare(
      "UPDATE smc_ac_label SET ac_id_tipo = :ac_tipo, ac_conteudo = :ac_conteudo WHERE ac_label_id = :ac_label_id AND ac_id_ac = :ac_id_ac"
    );
    $stmtUpdate->execute([
      ':ac_tipo' => $_POST['ac_tipo'],
      ':ac_conteudo' => $_POST['ac_conteudo'],
      ':ac_label_id' => $_POST['ac_id'],
      ':ac_id_ac' => $_POST['ac_id_ac']
    ]);

    $updateGoTo = "planejamento_editar.php";
    if (isset($_SERVER['QUERY_STRING'])) {
  ...
  $stmtEtapa->execute([':etapa_id' => $etapa]);
  ...
  $stmtComponente = $SmecelNovo->prepare(
    "SELECT disciplina_id, disciplina_area_conhecimento_id, disciplina_codigo_inep, disciplina_ordem, disciplina_nome, 
        disciplina_nome_abrev, disciplina_cor_fundo, disciplina_bncc, disciplina_diversificada, disciplina_id_campos_exp 
        FROM smc_disciplina WHERE disciplina_id = :disciplina_id"
  );
  $stmtComponente->execute([':disciplina_id' => $componente]);
  ...
  $stmtBnccEf = $SmecelNovo->prepare(
    "SELECT bncc_ef_id, bncc_ef_area_conhec_id, bncc_ef_comp_id, bncc_ef_componente, bncc_ef_ano, bncc_ef_campos_atuacao, 
        bncc_ef_eixo, bncc_ef_un_tematicas, bncc_ef_prat_ling, bncc_ef_obj_conhec, bncc_ef_habilidades, bncc_ef_comentarios, bncc_ef_poss_curr 
        FROM smc_bncc_ef WHERE bncc_ef_comp_id = :disciplina AND bncc_ef_ano IN ($etapa_ano)"
  );
  $stmtBnccEf->execute([':disciplina' => $row_Componente['disciplina_id']]);
  $bnccEf = $stmtBnccEf->fetchAll(PDO::FETCH_ASSOC);
  $totalRows_bncc_ef = $stmtBnccEf->rowCount();
...
<body>
  ...
  <main class="ls-main">
    <div class="container-fluid">
      ...
      <p><a
          href="planejamento_editar.php?ac=<?= $row_ac['ac_id'] ?>&escola=<?= $row_ac['ac_id_escola'] ?>&etapa=<?= $row_ac['ac_id_etapa'] ?>&componente=<?= $row_ac['ac_id_componente'] ?>"
        ...
        <fieldset>
          <label class="ls-label col-md-6">
            ...
            <div class="ls-custom-select">
              <select class="ls-custom" name="ac_tipo" required>
                <option>SELECIONAR</option>
                <option value="99" <?php if (!(strcmp(99, htmlentities($rowAcLabel['ac_id_tipo'], ENT_COMPAT, 'utf-8')))) {
                  echo "SELECTED";
                } ?>>Planejamento completo</option>
                <option value="1" <?php if (!(strcmp(1, htmlentities($rowAcLabel['ac_id_tipo'], ENT_COMPAT, 'utf-8')))) {
                  echo "SELECTED";
                } ?>>Unidade temática</option>
                <option value="2" <?php if (!(strcmp(2, htmlentities($rowAcLabel['ac_id_tipo'], ENT_COMPAT, 'utf-8')))) {
                  echo "SELECTED";
                } ?>>Objetivos de Aprendizagem e desenvolvimento</option>
                <option value="3" <?php if (!(strcmp(3, htmlentities($rowAcLabel['ac_id_tipo'], ENT_COMPAT, 'utf-8')))) {
                  echo "SELECTED";
                } ?>>Objetos de conhecimento/saberes e conhecimento/conteúdo</option>
                <option value="4" <?php if (!(strcmp(4, htmlentities($rowAcLabel['ac_id_tipo'], ENT_COMPAT, 'utf-8')))) {
                  echo "SELECTED";
                } ?>>Habilidades</option>
                <option value="5" <?php if (!(strcmp(5, htmlentities($rowAcLabel['ac_id_tipo'], ENT_COMPAT, 'utf-8')))) {
                  echo "SELECTED";
                } ?>>Metodologia</option>
                <option value="6" <?php if (!(strcmp(6, htmlentities($rowAcLabel['ac_id_tipo'], ENT_COMPAT, 'utf-8')))) {
                  echo "SELECTED";
                } ?>>Avaliação</option>
                <option value="7" <?php if (!(strcmp(7, htmlentities($rowAcLabel['ac_id_tipo'], ENT_COMPAT, 'utf-8')))) {
                  echo "SELECTED";
                } ?>>Observação</option>
                <option value="8" <?php if (!(strcmp(8, htmlentities($rowAcLabel['ac_id_tipo'], ENT_COMPAT, 'utf-8')))) {
                  echo "SELECTED";
                } ?>>Recursos</option>
              </select>
            </div>
