...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    error_log("Dados recebidos - plan_id: $plan_id, professor_id: $professor_id, escola_id: $escola_id, turma_id: $turma_id, componente_id: $componente_id, observacao: $observacao");

    // Verificar se já existe um planejamento para essa turma e componente
    $stmtCheck = $SmecelNovo->prepare("
        SELECT COUNT(*) 
        FROM smc_planejamento 
        WHERE smc_id_turma = :turma_id AND smc_id_componente = :componente_id AND smc_id_planejamento != :plan_id
    ");
    $stmtCheck->execute([
        ':turma_id' => $turma_id,
    ...
    if ($stmtCheck->fetchColumn() > 0) {
        die("Erro: Já existe um planejamento para essa turma e componente.");
    }
    ...
    $stmtInsert->execute([
        ...
        ':data_inicial' => $original['smc_planejamento_data_inicial'],
        ':data_final' => $original['smc_planejamento_data_final'],
