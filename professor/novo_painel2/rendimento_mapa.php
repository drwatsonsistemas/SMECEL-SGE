...
<body>
          ...
          <thead>
            <tr class="">
              ...
              <?php for ($p = 1; $p <= $row_Criterios['ca_qtd_periodos']; $p++) { ?>
                ...
                ?>
                <th colspan="<?php echo $row_Criterios['ca_qtd_av_periodos'] + 1; ?>" class="ls-txt-center " width="15">
                  <?php echo $p; ?>º PERÍODO <br><small>LIMITE:
                    ...
                    <?php if (($totalRows_PeriodosBloqueio1 > 0) && ($row_PeriodosBloqueio1['per_unid_data_bloqueio'] < date("Y-m-d"))) { ?>
                      <br><span class="">Período atingido</span> <?php } ?></small>
                </th>
              <?php } ?>
              ...
              <tr>
                ...
                <?php for ($p = 1; $p <= $row_Criterios['ca_qtd_periodos']; $p++) { ?>
                  ...
                  <?php
                    ...
                    $stmt_PeriodosBloqueio->bindParam(':sec_id', $row_Secretaria['sec_id'], PDO::PARAM_INT);
                    $stmt_PeriodosBloqueio->bindParam(':periodo', $p, PDO::PARAM_INT);
                    $stmt_PeriodosBloqueio->bindParam(':ano_letivo_id', $row_AnoLetivo['ano_letivo_id'], PDO::PARAM_INT);
                  ...
                  <?php for ($a = 1; $a <= $row_Criterios['ca_qtd_av_periodos']; $a++) { ?>
                    <td width="15" class="ls-txt-center "><?php
                     $query_Notas = "
                  ...
                  <td width="15" class="ls-txt-center "><span
                      class="ls-text-md"><?php $mu = mediaUnidade($ru, $row_Criterios['ca_arredonda_media'], $row_Criterios['ca_aproxima_media'], $row_Criterios['ca_media_min_periodo'], $row_Criterios['ca_calculo_media_periodo'], $row_Criterios['ca_qtd_av_periodos'], $row_Criterios['ca_digitos']); ?></span>
                    <?php $tmu = $tmu + $mu; ?></td>
                <?php } ?>
                </td>
                <td width="15" class="ls-txt-center "><span
                    class="ls-text-md"><?php $tp = totalPontos($tmu, $row_Criterios['ca_digitos']); ?></span></td>
                <td width="15" class="ls-txt-center "><span
                    class="ls-text-md"><?php $mc = mediaCurso($tp, $row_Criterios['ca_arredonda_media'], $row_Criterios['ca_aproxima_media'], $row_Criterios['ca_min_media_aprovacao_final'], $row_Criterios['ca_qtd_periodos'], $row_Criterios['ca_digitos']); ?></span>
                </td>
                <td width="15" class="ls-txt-center "><?php
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function () {
      ...
      <?php }elseif($row_Criterios['ca_digitos'] == 2){ ?>
