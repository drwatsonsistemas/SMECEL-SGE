...
$query_ProfLogado = sprintf("SELECT func_id, func_nome, func_email, func_foto, func_sexo FROM smc_func WHERE func_id = %s", GetSQLValueString($colname_ProfLogado, "text"));
...
mysql_select_db($database_SmecelNovo, $SmecelNovo);
$query_Vinculos = "
SELECT vinculo_id, vinculo_id_escola, vinculo_id_sec, vinculo_id_funcionario, sec_id, sec_prefeitura 
FROM smc_vinculo 
IINER JOIN smc_sec ON sec_id = vinculo_id_sec
WHERE vinculo_id_funcionario = '$row_ProfLogado[func_id]'";
...
define("ANO_LETIVO", $row_AnoLetivo['ano_letivo_ano']);
