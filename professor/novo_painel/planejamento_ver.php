...
<body>
	...
	<main class="ls-main">
		<div class="container-fluid">
			...
			<p><a href="planejamento_mapa.php" class="ls-btn ls-ico-chevron-left">Voltar</a> <a
					href="planejamento_mapa.php" class="ls-btn-primary">NOVO PLANEJAMENTO</a>
			</p>
								...
								<tbody>
									<?php foreach ($resultNewPlan as $plan) { ?>
										<tr id="linhav2-<?php echo $plan['smc_id_planejamento']; ?>">
											...
											<td class="ls-txt-center">
												<?php 
												$turma_nome = mb_convert_encoding($plan['turma_nome'], 'UTF-8', 'UTF-8');
												echo htmlspecialchars($turma_nome, ENT_QUOTES, 'UTF-8');
												//echo htmlspecialchars($plan['turma_nome'], ENT_QUOTES, 'UTF-8'); ?>
											</td>
											<td class="ls-txt-center">
		...
		<div class="ls-modal-box">
			...
			<div class="ls-modal-body">
				<form id="formDuplicarPlanejamento" method="POST"
					action="crud/planejamento/duplicar_planejamento_quinzenal.php" class="ls-form ls-form-horizontal">
					...
					<div class="ls-row">
						<div class="col-md-6">
							<label class="ls-label">
								<span class="ls-label-text">Data Inicial</span>
								<input type="date" name="data_inicial" id="data_inicial" class="ls-field" required>
							</label>
						</div>
						<div class="col-md-6">
							<label class="ls-label">
								<span class="ls-label-text">Data Final</span>
								<input type="date" name="data_final" id="data_final" class="ls-field" required>
							</label>
						</div>
					</div>
				</form>
	...
	<script>
		document.querySelectorAll('.duplicar-planejamento').forEach(button => {
			button.addEventListener('click', function (e) {
				...
				// Buscar as datas do planejamento original
				fetch(`consultas/get_planejamento_datas.php?plan_id=${planId}`)
		...
		function carregarTurmas() {
			...
			console.log('Parâmetros enviados:', escolaId, professorId, '<?php echo ANO_LETIVO; ?>');
				...
				.then(text => {
					...
					try {
						...
						turmas.forEach(turma => {
							// Substitui "º" por "&ordm;" para garantir a exibição correta
							const nomeTurma = turma.turma_nome.replace(/º/g, '&ordm;');
							turmaSelect.innerHTML += `<option value="${turma.turma_id}">${nomeTurma} (${turma.turma_turno_nome})</option>`;
						});
					} catch (e) {
