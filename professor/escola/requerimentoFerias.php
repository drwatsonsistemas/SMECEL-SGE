...
func_endereco, func_endereco_numero, func_endereco_bairro, func_endereco_uf, func_endereco_cidade, func_regime, func_estado_civil
FROM smc_vinculo
...
if (isset($_GET['data_ferias_inicio']) && $_GET['data_ferias_inicio'] != '' AND isset($_GET['data_ferias_final']) && $_GET['data_ferias_final'] != '' AND isset($_GET['ano_referencia']) && $_GET['ano_referencia'] != '' AND isset($_GET['c']) && $_GET['c'] != '') {
  function formatarIntervaloPorExtenso($dataInicio, $dataFinal)
  {
    // Mapear números do mês para os nomes por extenso
    $meses = array(
      '01' => 'janeiro',
      '02' => 'fevereiro',
      '03' => 'março',
    ...
    // Quebrar as strings de data em ano, mês e dia
    list($anoInicio, $mesInicio, $diaInicio) = explode('-', $dataInicio);
    list($anoFinal, $mesFinal, $diaFinal) = explode('-', $dataFinal);

    // Formatar as datas por extenso
    $dataInicioPorExtenso = $diaInicio . ' de ' . $meses[$mesInicio] . ' de ' . $anoInicio;
    $dataFinalPorExtenso = $diaFinal . ' de ' . $meses[$mesFinal] . ' de ' . $anoFinal;

    return array(
      'inicio' => $dataInicioPorExtenso,
      'final' => $dataFinalPorExtenso
    );
  }

  // Exemplo de uso
  $dataInicio = $_GET['data_ferias_inicio'];
  $dataFinal = $_GET['data_ferias_final'];

  $intervaloPorExtenso = formatarIntervaloPorExtenso($dataInicio, $dataFinal);
} else {
...
<div class="container-fluid"> 
  ...
  <div class="col-xs-12 ls-txt-center">
	...
	<p><h1>SOLICITAÇÃO DE FÉRIAS</h1></p><br>
...
<p style="line-height: 180%; text-align:justify; font-size:16px;">
<b>Assunto:</b> Solicitação de férias.
</p>
<br><br><br>       
        
  <p style="line-height: 180%; text-align:justify; font-size:16px;">

  Eu, <b><?= $row_FrequenciaFuncionario['func_nome'] ?></b>, brasileiro, <?= $estado_civil ?>, <?= $regime ?> 
  para <?= $row_FrequenciaFuncionario['funcao_nome'] ?>, 
  ...
  respeitosamente solicito a Vossa Senhoria o gozo de férias regulamentares referente ao período aquisitivo 
  de <?= $_GET['ano_referencia'] ?>, que será usufruída no período de <?= $intervaloPorExtenso['inicio'] ?>  a  <?= $intervaloPorExtenso['final'] ?>.
  <br><br>
