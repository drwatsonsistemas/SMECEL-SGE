...
FROM smc_turma 
WHERE turma_tipo_atendimento = '1' AND turma_id_escola = '$row_UsuLogado[usu_escola]' AND turma_ano_letivo = '$anoLetivo' 
...
<body>
	...
	<main class="ls-main ">
		<div class="container-fluid">
			...
			<h1 class="ls-title-intro ls-ico-home">Alunos vinculados na escola no <strong>Ano Letivo <?php echo $anoLetivo; ?></strong></h1>
			<!-- CONTEÚDO -->
			...
			<div class="ls-box-filter">
				...
				<div data-ls-module="dropdown" class="ls-dropdown">
					...
					<ul class="ls-dropdown-nav">
						...
						<li><a href="print_vinculoAlunoExibirMatriculaInicial.php<?php if ($codTurma <> "") { echo "?ct=$codTurma"; } ?><?php if ($stCod <> "") { echo $sep."st=$stCod"; } ?>" target="_blank">Alunos - Matrícula inicial</a></li>
						<li><a href="print_vinculoAlunoExibirTurmaMatricula.php<?php if ($codTurma <> "") { echo "?ct=$codTurma"; } ?><?php if ($stCod <> "") { echo $sep."st=$stCod"; } ?>" target="_blank">Por data de matrícula</a></li>
						<li><a href="print_vinculoAlunoExibirTurmaIdade.php<?php if ($codTurma <> "") { echo "?ct=$codTurma"; } ?><?php if ($stCod <> "") { echo $sep."st=$stCod"; } ?>" target="_blank">Turma/Idade dos Alunos</a></li>
						<li><a href="print_vinculoAlunoExibirTurmaTransporteEscolar.php<?php if ($codTurma <> "") { echo "?ct=$codTurma"; } ?><?php if ($stCod <> "") { echo $sep."st=$stCod"; } ?>" target="_blank">Transporte Escolar - Todos</a></li>
						<li><a href="print_vinculoAlunoExibirTurmaTransporteEscolarRural.php<?php if ($codTurma <> "") { echo "?ct=$codTurma"; } ?><?php if ($stCod <> "") { echo $sep."st=$stCod"; } ?>" target="_blank">Transporte Escolar - Rural</a></li>
						<li><a href="print_vinculoAlunoExibirTurmaTransporteEscolarUrbana.php<?php if ($codTurma <> "") { echo "?ct=$codTurma"; } ?><?php if ($stCod <> "") { echo $sep."st=$stCod"; } ?>" target="_blank">Transporte Escolar - Urbana</a></li>
						...
						<li><a href="print_boletimTurma.php<?php if ($codTurma <> "") { echo "?ct=$codTurma"; } ?>" target="_blank">Boletim</a></li>
						<li><a href="print_fichaIndividualTurma.php<?php if ($codTurma <> "") { echo "?ct=$codTurma"; } ?><?php if ($stCod <> "") { echo $sep."st=$stCod"; } ?>" target="_blank">Ficha individual - por turma</a></li>
						...
						<li><a href="print_carteirinhaAlunoZonaRural.php<?php if ($codTurma <> "") { echo "?ct=$codTurma"; } ?><?php if ($stCod <> "") { echo $sep."st=$stCod"; } ?>" target="_blank">Carteirinha ZONA RURAL</a></li>
						...
						<li><a href="print_alunos_faltosos.php<?php if ($codTurma <> "") { echo "?ct=$codTurma"; } ?><?php if ($stCod <> "") { echo $sep."st=$stCod"; } ?>" target="_blank">Relação de alunos com total de faltas</a></li>
						<li><a href="print_alunos_ocorrencia.php<?php if ($codTurma <> "") { echo "?ct=$codTurma"; } ?><?php if ($stCod <> "") { echo $sep."st=$stCod"; } ?>" target="_blank">Relação de alunos com ocorrência</a></li>
						<li><a href="print_alunos_periodo_suspensao.php<?php if ($codTurma <> "") { echo "?ct=$codTurma"; } ?><?php if ($stCod <> "") { echo $sep."st=$stCod"; } ?>" target="_blank">Relatório de alunos em período de suspensão</a></li>
						...
						<li><a href="print_lista_entrega_kit.php<?php if ($codTurma <> "") { echo "?ct=$codTurma"; } ?><?php if ($stCod <> "") { echo $sep."st=$stCod"; } ?>" target="_blank">Recibo de entrega de Kit Merenda</a></li>
						<li><a href="print_lista_entrega_kit_escolar.php<?php if ($codTurma <> "") { echo "?ct=$codTurma"; } ?><?php if ($stCod <> "") { echo $sep."st=$stCod"; } ?>" target="_blank">Recibo de entrega de Kit Escolar</a></li>
				...
				<div data-ls-module="dropdown" class="ls-dropdown ls-float-right1">
					...
					<ul class="ls-dropdown-nav">
						...
						<li><a href="vinculoAlunoExibirTurma.php?ano=<?php echo $row_AnoLetivo['ano_letivo_ano'];; ?>" target="" title="Diários">ANO LETIVO <?php echo $row_AnoLetivo['ano_letivo_ano']; ?></a></li>

						<?php do { ?>

							<li><a href="vinculoAlunoExibirTurma.php?ano=<?php echo $row_Ano['ano_letivo_ano']; ?>" target="" title="Diários">ANO LETIVO <?php echo $row_Ano['ano_letivo_ano']; ?></a></li>
			...
			<div class="ls-box">
				...
				<form id="form_busca" autocomplete="off" action="redireciona.php" method="get" class="ls-form ls-form-inline row">
					<label class="ls-label col-md-12">
						...
						<input id="inputString" ano="<?php echo $anoLetivo; ?>" type="text" class="validate" value="" placeholder="BUSQUE PELO NOME DO ALUNO OU O NOME DA MÃE DO ALUNO MATRICULADO EM <?php echo $anoLetivo; ?>" onkeyup="lookup(this.value);" onblur="fill();" autofocus />
				...
				<div class="ls-box">
					...
					<header class="ls-info-header">
						...
						<p>ID DA TURMA: <b><?php echo $row_ExibirTurmas['turma_id']; ?></b><br> 
							...
							TURMA MULTI: <?php if (($row_ExibirTurmas['turma_multisseriada']==1)) { ?> <b>SIM</b> <?php } else { ?><b>NÃO</b><?php } ?><p>
								...
								<table class="ls-table ls-table-striped ls-sm-space fonte-tabela">
									<thead>
										<tr>
											...
											<th class="ls-txt-center  hidden-xs">LOCALIDADE</th>
											<th class="ls-txt-center hidden-xs" width="45">PNE</th>
										...
										<tbody>
											<?php do { ?>
												...
												<tr>
													...
													$contaAlunos++;		
												?></td>
												<td class="ls-txt-center"><a href="matriculaExibe.php?cmatricula=<?php echo $row_ExibirAlunosVinculados['vinculo_aluno_hash']; ?>&ano=<?php echo $anoLetivo; ?>"><?php echo str_pad($row_ExibirAlunosVinculados['vinculo_aluno_id'], 5, '0', STR_PAD_LEFT); ?></a></td>
											...
											<td class="ls-txt-center  hidden-xs">
												<?php
												if($row_ExibirAlunosVinculados['aluno_localizacao'] == 1){
													echo "URBANA";
												}elseif ($row_ExibirAlunosVinculados['aluno_localizacao'] == 2) {
													echo "RURAL";
												}else{
													echo '';
												}
											...
											<td class="ls-txt-center hidden-xs"><?php if ($row_ExibirAlunosVinculados['aluno_aluno_com_deficiencia']==1) { ?><img src="../../img/pne.png" width="20px" style="cursor:pointer" alt="Aluno(a) com necessidades especiais" title="Aluno(a) com necessidades especiais: <?php echo $row_ExibirAlunosVinculados['aluno_tipo_deficiencia']; ?>"> <?php } ?></td>
											<td class="ls-txt-center hidden-xs"><?php echo $row_ExibirAlunosVinculados['aluno_cod_inep']; ?></td>
											<td class="ls-txt-center hidden-xs"><?php if ($row_ExibirAlunosVinculados['vinculo_aluno_boletim']=="0") { echo "<span class=\"ls-tag-warning\">NÃO</span>"; } else { echo "<span class=\"ls-tag-success\">SIM</span>"; } ?></td>
											...
											<td class="ls-txt-center "><?php echo $row_ExibirAlunosVinculados['vinculo_aluno_situacao_nome']; ?></td>
