...
$query_Alunos = "
SELECT vinculo_aluno_id, vinculo_aluno_id_aluno, aluno_nome, turma_matriz_id, matriz_criterio_avaliativo
...
if ($totalRows_Alunos > 0) {
    while ($row_Alunos = mysql_fetch_assoc($Alunos)) {
        ...
        while ($row_disciplinas = mysql_fetch_assoc($disciplinas)) {
            ...
            for ($p = 1; $p <= 4; $p++) { // Exemplo: 4 períodos
                for ($a = 1; $a <= 3; $a++) { // Exemplo: 3 avaliações por período
