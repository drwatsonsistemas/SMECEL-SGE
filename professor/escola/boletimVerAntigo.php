...
// ** REGISTRO DE LOG DE USUÁRIO **
  ...
  $insertGoTo = "boletimVerAntigo.php?cadastrada";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_SmecelNovo, $SmecelNovo);
$query_EscolaLogada = "SELECT escola_id, escola_id_sec, escola_nome, escola_cep, escola_endereco, escola_num, escola_bairro, escola_telefone1, escola_telefone2, escola_email, escola_cnpj, escola_inep, escola_tema FROM smc_escola WHERE escola_id = '$row_UsuLogado[usu_escola]'";
      ...
      <tbody>
        <?php do { ?>
          <tr>
            ...
            <td class="ls-txt-center"><strong><a href="editarNotas.php?cd=<?php echo $row_Disciplinas['boletim_id']; ?>&c=<?php echo $colname_Boletim; ?>&n=af" class="ls-btn-xs ls-btn-block<?php if (($row_Disciplinas['boletim_af']<$row_CriteriosAvaliativos['ca_nota_min_recuperacao_final'])&&($row_Disciplinas['boletim_af']>0)) { echo " ls-btn-primary-danger"; } ?><?php if ($row_Disciplinas['boletim_af']>=$row_CriteriosAvaliativos['ca_nota_min_recuperacao_final']) { echo " ls-btn-primary"; } ?>"><?php echo $af = avaliacaoFinalLancamentoBoletim($row_Disciplinas['boletim_af'],$row_CriteriosAvaliativos['ca_nota_min_recuperacao_final']); ?></a></strong></td>
