...
if ($totalRows_Turma==0) {
	...
	echo "<script>alert('Este processo só está habilitado para turmas de Educação Infantil');</script>";

  echo "
  <br>
  <p style='text-align:center; float:center;'><a href='turmaListar.php' style='padding:5px; background-color:#000000; color:white; text-align:center; float:center; text-decoration:none'>VOLTAR</a></p>
  ";
...
$query_Alunos = sprintf("
  ...
  WHERE etapa_id_filtro = '1' AND vinculo_aluno_boletim = '0' AND vinculo_aluno_id_escola = $row_EscolaLogada[escola_id] AND vinculo_aluno_id_turma = %s", GetSQLValueString($colname_Alunos, "int"));
...
mysql_select_db($database_SmecelNovo, $SmecelNovo);
$query_Criterios = "SELECT ca_id, ca_id_secretaria, ca_descricao, ca_qtd_periodos, ca_qtd_av_periodos, ca_nota_min_av, ca_nota_max_av, ca_calculo_media_periodo, ca_media_min_periodo, ca_arredonda_media, ca_aproxima_media, ca_min_pontos_aprovacao_final, ca_min_media_aprovacao_final, ca_nota_min_recuperacao_final, ca_detalhes, ca_rec_paralela, ca_questionario_conceitos FROM smc_criterios_avaliativos WHERE ca_id = '$row_Alunos[matriz_criterio_avaliativo]'";
...
if ($row_Criterios['ca_questionario_conceitos']=="S") {
	
	echo "<script>alert('Este processo não está habilitado para turmas de Ensino Fundamental com avaliação por conceito');</script>";

  echo "
  <br>
  <p style='text-align:center; float:center;'>No momento, só é permitido gerar boletins para turma de Ensino Fundamental que são avaliados por notas.</p>
  <p style='text-align:center; float:center;'><a href='turmaListar.php' style='padding:5px; background-color:#000000; color:white; text-align:center; float:center; text-decoration:none'>VOLTAR</a></p>
  ";
      ...
      <?php if ($totalRows_Alunos > 0) { ?>
        ...
        <table class="ls-table">
          ...
          <?php do { ?>
            ...
            $updateSQL = sprintf("UPDATE smc_vinculo_aluno SET vinculo_aluno_boletim='1' WHERE vinculo_aluno_id=%s",
...
//	exit; 
            ...
            <tr>
              ...
              <td>
                ...
                <span class="ls-ico-checkmark-circle ls-color-success"> BOLETIM GERADO</span>
