...
//$anoLetivo = $row_AnoLetivo['ano_letivo_ano'];
	...
	$query_AnoLetivo = "AND ano_letivo_ano = $anoLetivo";
}


mysql_select_db($database_SmecelNovo, $SmecelNovo);
$query_AnoLetivo = "SELECT ano_letivo_id, ano_letivo_ano, ano_letivo_inicio, ano_letivo_fim, ano_letivo_aberto, ano_letivo_id_sec, ano_letivo_data_rematricula, ano_letivo_resultado_final, ano_letivo_mat_inicial, ano_letivo_mat_final FROM smc_ano_letivo WHERE ano_letivo_id_sec = '$row_UsuLogado[usu_sec]' $query_AnoLetivo ORDER BY ano_letivo_ano DESC LIMIT 1";
...
<title>FREQUENCIA ESCOLAR <?php echo $row_AlunoBoletim['vinculo_aluno_ano_letivo'] ." - ". $row_AlunoBoletim['turma_nome'] ." - ". $row_EscolaLogada['escola_nome'] ?></title>
<meta charset="utf-8">
				...
				?>
				  ...
				  ?>
                    ...
                    <span class="ls-float-right" style="margin-left:20px;">
					  ...
					  <?php if ($row_AlunoBoletim['vinculo_aluno_situacao']<>"1") { ?><br><strong>Matrícula encerrada em <?php echo date("d/m/Y", strtotime($row_AlunoBoletim['vinculo_aluno_datatransferencia'])); ?></strong><?php } ?>                      </small>
