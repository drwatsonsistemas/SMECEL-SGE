...
<head>
	...
	<style>
		...
		.background-image {
			...
			background-image: url('<?php echo URL_BASE.'img/cartao_cdi.png'; ?>');
		...
		.text-container-sanguineo {
			position: absolute; /* Posiciona o elemento em relação ao contêiner pai */
			top: 41.5%; /* Alinha verticalmente com a foto */
			left: calc(18% + 31.5mm); /* Posiciona horizontalmente ao lado da foto */
			transform: translateY(-50%); /* Ajusta a posição para centrar verticalmente em relação à foto */
			width: calc(54mm - 14mm - 8mm); /* Ajuste conforme necessário para caber ao lado da foto */
			padding-left: 10px; /* Espaço entre a foto e o texto */
			box-sizing: border-box;
...
<body onload="self.print();">
	...
	<?php do { ?>
		...
		aluno_id, aluno_nome, aluno_nome_social, aluno_nascimento, aluno_filiacao1, aluno_hash, aluno_foto, aluno_filiacao2, aluno_laudo, aluno_aluno_com_deficiencia,aluno_identidade,aluno_cpf,aluno_def_autista,aluno_cid,aluno_hash,aluno_tel_pai,aluno_tel_mae,aluno_sangue_tipo,aluno_sangue_rh
				...
				do { 
					...
					<div class="background-image" style="float: left; margin: 10px;">
						...
						<div class="photo">
							...
							<?php } else { ?>
								<img src="<?php echo URL_BASE.'aluno/fotos/' ?><?php echo $row_ExibirAlunosVinculados['aluno_foto']; ?>" class="aluno" style="margin:1mm;width:15mm;">
							<?php } ?>  
						</div>
						<div class="text-container">
							<p style="font-size:8px; color: #0F338B;font-weight: bold;">Aluno(a): <?php echo $row_ExibirAlunosVinculados['aluno_nome_social'] != ""? $row_ExibirAlunosVinculados["aluno_nome_social"] : $row_ExibirAlunosVinculados["aluno_nome"]; ?></p>
						...
						<div class="text-container-sanguineo" style="font-weight:bold;color: white">
							<p style="font-size:10px"><?php echo $row_ExibirAlunosVinculados['aluno_sangue_tipo'].$row_ExibirAlunosVinculados['aluno_sangue_rh']; ?></p>
						</div>
